...
> ## 📋 变更记录 (Changelog)
>
> - **2026-07-29 (V0.3.35 boohee module — ENDPOINTS.boohee + FeatureFlag boohee)** — 🎯 **packages/shared V0.3.35 增量**（2 处改动）：① **ENDPOINTS.boohee** 新增 7 action（search/detail/categories/categoryFoods/foodUnits/batchNutrition/foodRanking），顶层 keys 32→**33**；② **FeatureFlag 'boohee'** 新增（FEATURE_FLAGS 数组 + FeatureFlagsConfig interface，默认 false 远程开启）；**types/index.ts 0 改动**；**实测**：packages/shared it() **6**（V0.3.35 0 新测）；**关键范式**：boohee 沿用 generic module 模式（ENDPOINTS 显式登记 7 action，与 strength V0.2.42 同范式）；**miniprogram_npm rebuild 已执行**（build-mp-shared.mjs → 10 文件含 boohee endpoints.js + feature-flags.js）；**待办**：① 建议补 endpoints.test.ts 校验 boohee.* 7 action 字符串存在；② 生产 .env 注入 BOOHEE_API_KEY
>
> - **2026-07-29 (init #23 V0.3.34 校准 + shared V0.3.25-34 零改动声明)** — 🎯 **packages/shared V0.3.25-34 零代码改动**（10 tags，**shape only 原则保持**）：① V0.3.25-29 全是 apps/miniprogram UI + 后端 stats 缓存修复，不动 shared；② V0.3.30-33 qm-admin 独立仓改动，不动主仓 shared；③ **V0.3.34 sprint A admin MIS 8 子项沿用 generic action 模式**（A2 getUserDetail / A3 batchUpdateOrderStatus+batchRefundOrder / A4 globalSearch types filter / A5 dashboard dailyTrend / A6 exportOrdersExcel / A7 listAuditLogs targetType / A8 config 远程热更新 — 全部后端 admin generic action，shared ENDPOINTS.admin 不单独登记，与 V0.2.37 listInterpret + V0.3.4/5 dashboard/globalSearch 同模式）；**ENDPOINTS 现状**：顶层 keys **32**（init #22 沿用，V0.3.25-34 0 改动）/ 36 module 中 recipe+ludong stub + admin/device generic action 不计 ENDPOINTS；**types/index.ts / constants/* 0 改动**；**实测**：packages/shared it() **6**（endpoints.test.ts，V0.3.25-34 0 新测）；**关键范式沉淀**：**admin generic action 模式持续应用**（V0.3.34 sprint A 8 子项全部不在 ENDPOINTS 显式登记，沿用 V0.2.37 以来范式，避免 ENDPOINTS 膨胀）；**GAP-12 36/36 保持**（shared ENDPOINTS 0 改动）；**待办**：① NotifType enum 补全 8 值（init #21/#22 延续待办，V0.2.125 new_post / V0.2.129 plan_completed 后端已用，shared 仍只 6 旧值）；② miniprogram_npm rebuild（ENDPOINTS.goal +4 action 待办延续，V0.3.34 无新增但 V0.3.9+16 待办仍在）；③ 建议补 `endpoints.test.ts` 校验 goal.* 4 新 action 字符串存在（init #22 待办延续）
>
> - **2026-07-28 (init #22 V0.3.24 校准收官 + ENDPOINTS.goal +4 action)** — 🎯 **packages/shared V0.3.6-24 增量**（**仅 ENDPOINTS.goal +4 action + device 注释**，保持「shape only」原则）：① **V0.3.9 ENDPOINTS.goal +3 action**：`updateProgress` / `pauseGoal` / `resumeGoal`（V0.3.12 hotfix 显式登记，按 V0.2.61 范式；前端 pages/goal 调用会 fallback 到 /api/goal 通用路径但脏代码必须显式登记）；② **V0.3.16 ENDPOINTS.goal +1 action**：`recommend`（Phase 6 系统推荐 8 规则引擎，RecommendationOutputSchema）；③ **V0.3.20 device 注释**：authCenterList/recentActivityByVendor action **沿用 admin generic action 模式不单独登记**（与 V0.2.37 listInterpret + V0.3.4/5 admin dashboard/globalSearch 同模式，device ENDPOINTS 不加）；**ENDPOINTS 现状 36 module 保持**（goal 块 4→8 action = list/add/remove/myProgress/addFamilyGoal/myFamilyGoals + 4 customMilestone + V0.3.9 3 + V0.3.16 1 recommend）；**types/index.ts / constants/* 0 改动**；**实测**：packages/shared it() **6**（endpoints.test.ts，V0.3.x 0 新测）；**关键范式沉淀**：① **ENDPOINTS.goal action 显式登记**（避免前端调用 fallback 到 /api/goal 通用路径的脏代码，V0.2.61 范式持续应用）；② **admin/device generic action 模式**（authCenterList/recentActivityByVendor 不在 ENDPOINTS 显式登记，沿用 admin 同模式，避免 ENDPOINTS 膨胀）；**GAP-12 36/36 保持**（shared ENDPOINTS 36 module 不变）；**待办**：① miniprogram_npm rebuild（ENDPOINTS.goal +4 action 下次部署前必 rebuild）；② 建议补 `endpoints.test.ts` 校验 goal.* 4 新 action 字符串存在（V0.3.x 未加测试，coverage 风险点）；③ NotifType enum 补全 8 值（init #21 已记待办，V0.2.125 new_post / V0.2.129 plan_completed 后端已用，shared 仍只 6 旧值，下轮 init 补）
>
> - **2026-07-27 (init #21 V0.3.5：shared 0 代码改动声明)** — 🎯 **V0.3.1-5 期间 packages/shared 源代码 0 改动**：① V0.3.1 device-poll-pull.job.ts 多 vendor 扩展（DeviceVendor type / DailyData / DevicePollPullResult 都是后端 internal interface，不动 shared）；② V0.3.2 interpret 错误一致性（后端 throw Error → Errors.*，不动 shared）；③ V0.3.3 admin 错误一致性（后端，不动 shared）；④ V0.3.4 admin.dashboard（后端 generic action 模式，shared ENDPOINTS.admin 不单独登记，与 V0.2.37 listInterpret 同模式）；⑤ V0.3.5 admin.globalSearch（同 V0.3.4 模式，ENDPOINTS.admin 不加）；**ENDPOINTS 36 module 保持**（admin 沿用 generic action 模式，dashboard/globalSearch 不在 ENDPOINTS 显式登记）；**types/index.ts / constants/* 0 改动**；**实测**：packages/shared it() **6**（endpoints.test.ts）/ V0.3.x 0 新测；**待办**：① NotifType enum 补全 8 值（V0.2.125 new_post / V0.2.129 plan_completed 后端已用，shared 仍只 6 旧值，下轮 init 补）；② miniprogram_npm rebuild（V0.2.131+141/143/148/150/152 共 6 处 shared 增量，下次部署前必 rebuild）；**子 CLAUDE.md 落后段补齐说明**：packages/shared/CLAUDE.md 顶部落后根 CLAUDE.md 三段（V0.2.130-153 + V0.3.1-5），本轮 init #21 只 prepend 本声明段，V0.2.130-153 历史段不补（避免单次 init 改动过大）

> - **2026-07-25 (V0.2.119-129 11 版 sprint：shared 仅 1 处增 + 0 改)** — 🎯 **packages/shared 极小增量**（**保持「shape only」原则，shared 仅做契约不存业务**）：① **V0.2.126 ENDPOINTS.strength +`getExerciseStats` action**（`api-contracts/endpoints.ts` strength 块 +1 行：动作统计 PB + 容量分布 单一 action）；② V0.2.119-125 / V0.2.127-129 期间 **shared 0 改动**（V0.2.119-125 都是 backend `publishToUser` + 前端 toast 文案，V0.2.127-129 是后端 strength 字段 + 前端 UX）；**ENDPOINTS 现状 36 module 不变**（strength 块 7→8 action = startSession/addSet/finishSession/listSessions/sessionDetail/myVolume/listExercises/getExerciseStats）；**types/index.ts** 保持 `GrowthLevel` + `AdminRole` + `MemberPackage` + `NotifType`（V0.2.119-129 后端用 8 个 type 但只 zod enum 校验输入端 + Prisma String 存；**shared NotifType enum 仍只导出 6 个 like/comment/follow/system/goal_achieved/strength_done 旧值**，V0.2.125 new_post / V0.2.129 plan_completed 只后端写入 + Prisma String 接受任意值，前端不验 Zod；下次 init 应加齐 NotifType 8 值）；**constants/member-levels.ts** 0 改；**待办**：① 补全 NotifType 8 值（V0.2.130+ init 校准补）；② miniprogram_npm rebuild（如 shared 有增量，本 V0.2.126 1 行理论应 rebuild，建议下次部署时一并）

- **2026-07-23 (init #19 V0.2.79 收官)** — 🎯 **`/zcf:init-project` 增量校准 #19**：本会话 init-architect 全量实测 + 主智能体 vitest cross-check（**apps/server it() = 1291 passed + 62 skipped = 1353 total** 权威数字） / scripts/dev-cli 16 / packages/shared 6 / **全仓 1313 合计** / **funcs 90.64%** / **36 module / 65 表 / 49 迁移 / 25 页 / 16 组件** / **GAP-12 36/36 closed**（V0.2.42 strength/CLAUDE.md 新建 + apps/flutter/CLAUDE.md 新建）；**V0.2.67-72 Flutter APP Phase 1-3**（apps/flutter ~80 文件，Riverpod 2.5 + go_router 14 + dio 5.7 + M3 + #2D9D78 + 4-tab + 21 feature + **与 apps/server 36 module API 全对齐**：ENDPOINTS.aiCoach 11 action / ENDPOINTS.sport 打卡 / ENDPOINTS.stats weatherAnalysis / ENDPOINTS.interpret V0.2.60-66 截图）/ **V0.2.73-79 测试加固 +97 测**（GAP-3.5 routes 全分流 + service 层深入 + funcs 列序认知修正 Stmts|Branch|Funcs|Lines 第 4 列=funcs）；**主智能体修正**：根 CLAUDE.md 中 ENDPOINTS 35→**36**（V0.2.42 +strength）+ module 索引 22 列表加 strength；**0 shared 源改动**（纯文档增量）；**miniprogram_npm 沿用 V0.2.42 rebuild 状态**（V0.2.33 ENDPOINTS.interpret 加后；V0.2.42 ENDPOINTS.strength 加后；V0.2.43~V0.2.79 shared 源 0 改动）；下一步：① interpret 真机验证（minimax key 注入 + 佳明 FIT 样本）；② 部署 v0.2.57 interpret screenshot；③ apps/flutter 微信 APP Phase 1.5 接入（待主人物料 + 签名）
