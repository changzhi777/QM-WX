...
| **GAP-14 funcs% 实跑** | ⚠️ **open（V0.2.11 init #14）** | 实跑发现 `pnpm test:coverage` 现 fail 22 个（admin.routes.test.ts 全 401 → 400 期望错，V0.2.8 RBAC 替换白名单 openid 时老测试未适配）；funcs% 仍 **无 fresh 数字**（vitest exit 1 无 coverage HTML）；GAP-14 修法 = 修 22 个 admin.routes.test.ts 适配 RBAC middleware（task owner 决策） |
