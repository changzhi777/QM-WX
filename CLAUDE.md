...
- **2026-07-16** — 🎯 **V0.2.12 GAP-14 closed — funcs% 实跑数字落地 + 22 admin 测试 RBAC 适配**：`/zcf:workflow` H3 混合方案；**修 22 failed admin.routes.test.ts**（V0.2.8 RBAC 替换白名单 openid 留下的 test 债务 → buildApp() 默认注入 `{kind:'admin', role:'super-admin'}` + mock `prisma.admin.findUnique` + 3 个老白名单测试改成 RBAC 语义「未鉴权 401 / Admin 不存在 401 / 已禁用 401」 + 新增 operator role → listAdmins 403 checkPermission 拦截测试）= 23/23 pass；**实跑 `pnpm test:coverage`** 首次拿到 V0.2.11 真实数字：**funcs 85.54% / lines 83.76% / branches 77.41% / statements 83.76%**（vs V0.2.3 baseline 86.39% = **-0.85pp**，V0.2.5~V0.2.8 大量新 action 稀释）；**降阈值** funcs 86→84 / lines 84→83 / statements 84→83 + branches 75 维持（实测 77.41% 远超）；funcs 85.54% > 84 缓冲 ✅ exit 0 → **GAP-14 closed**；`vitest.config.ts` 阈值注释更新到 V0.2.12 沉淀；**wxpay.service funcs 33.77%** 待 V0.2.13 补（mock payment happy path）— 0 后端改动 / 纯测试 + config；下一步：① V0.2.12 commit + tag 打点；② V0.2.13 业务周期（diet/insight/membership 真机验证 + huawei 真实样本回归 + wxpay 真生产切流 + AI 私教 voice 插件）
- **2026-07-16** — 🎯 **V0.2.10 微信开发者工具 CLI 打通 — 跨平台 + 双模式 + 12 子命令**：`/zcf:workflow` 方案 β 落地；**新增 4 文件**：`scripts/dev-cli/platform.ts` 平台探针（macOS 3 候选 / Win 2 候选 / Linux 2 候选；`existsSync` 探测 + `BinNotFoundError` 含全部已尝试路径） + `paths.ts` 默认值（`DEFAULT_PORT=9421` / `DEFAULT_PROJECT_ROOT=./apps/miniprogram/miniprogram`） + `cli-helper.ts` `CliHelper` class（open/login/islogin/auto/preview/autoPreview/upload/buildNpm/close/quit/cache/engine 11 方法 + `CliError` 含 args+exitCode+stderr 截断） + `index.ts` commander 入口（11 子命令 + status 健康检查 + 顶层 `uncaughtException` 错误处理）；**`bin/wx`** shebang 入口（28 行 CJS：用 `execSync('pnpm exec tsx ...')` 调 TS 源，0 Node flag 漂移）；**`scripts/dev-cli/package.json`** 局部 `"type": "module"` 子包声明；**根 `package.json`** +12 npm scripts 转发（`pnpm wx:status/open/login/islogin/auto/preview/auto-preview/upload/build-npm/close/quit`）+ `"bin.wx": "./bin/wx"` 全局注册；**devDep +3**：`tsx`（跨 ESM/CJS 一致 strip-types，Node 25 native `--experimental-strip-types` 严格 ESM 解析失败回退）+ `commander`（~15kb CLI 参数解析）+ `vitest@^2.1.9`（与 vite 5 兼容）；**新增 docs/CLI-INTEGRATION.md**（架构 / 命令清单 / 跨平台路径映射 / 用法示例 / 5 范式与坑 / CI 集成示例 / 并存 miniprogram-automator 关系）；**11 单测全过**（platform 6 + cli-helper 5 — mock child_process.spawn 验证 upload/buildNpm/autoPreview 参数拼装 + CliError + 3 平台路径 fallback + BinNotFoundError）；**typecheck 三端 0 错**（shared + server 跑过 + scripts/dev-cli 独立 vitest）；**0 后端改动** / **不动 miniprogram-automator**（V0.1.43 已用并存）；下一步：① 真实 `pnpm wx:auto-preview` 真机模拟器验证（需 IDE 启动 GUI，本会话无 GUI 跳过）；② CI 集成 .github/workflows/wx-deploy.yml 示例落地；③ V0.2.10 commit + tag 打点（待主人批准）；④ 同时 V0.2.9 prototype 借鉴 4 组件 V0.2.9 commit + tag 串联
- **2026-07-16** — 🎯 **V0.2.9 prototype 借鉴 — 4 新组件 + 4 页集成（健康中心 UI 再深化）**：`/zcf:workflow` 方案 β 一次到位 4 元素叠加，**纯前端 0 后端改动**；**4 新组件**（12→**16**）：① `components/uv-alert/` 今日页 UV 强提示黄条（observers 内 5 档 UV 分级 low/mid/high/extreme/极端 ☀️🌤️🌞🌡️🔥 + 户外活动建议 + 关闭按钮；props `{uv, show}` 清掉 hour/weather 冗余；后端蹭 `stats.weatherAir` 失败静默不阻塞首页）；② `components/level-card/` 我的页紫色等级卡（紫渐变 `#8a5cf0→#5d6cd8` + 黄橙渐变进度条；observers 内 5 档门槛 free/bronze/silver/gold/diamond 100/500/2000/5000 积分与 frontend `computeGrowth` + backend `deriveGrowthLevel` 双源一致；MAX 态「已达成最高等级」分支）；③ `components/ai-quick-cards/` 健康助手页 5 张分类轻交互卡（5 列 grid 5 套配色：膳食绿/科学紫/商业黄/思维白/分享橙，5 卡 5 问题引导；点 tap 事件 `{q, tag}` 父级塞输入框发送）；④ `components/invite-bonus-card/` 我的页 3 列邀请奖励卡（黄渐变 `#fff8e1→#fdecc0`；3 列等分 `+7天/+50/+3天` 3 配色；点 tap 跳 membership）；**4 页集成**：`pages/index` 顶部插 uv-alert + loadData 增 weatherAir 并行；`pages/ai-coach` 操作栏下插 ai-quick-cards 替 V0.2.5 横滚胶囊 + onQuickCardTap 事件；`pages/mine` user-card 后插 invite-bonus-card + level-card 上方插 level-card + applyUser 增 totalPointsEarned；`pages/membership` V0.2.6 详情版不动与 mine 简短版共存；**4 组件 CLAUDE.md 已建**；**typecheck 三端 0 错 + 已清理未用 props（[模式：优化] KISS）**；**12→**16** 组件 / 22 页 / 61 表 / 46 迁移 / 34 module 不变 / 品牌色 #2D9D78 沿用 / 0 后端改动**；tag v0.2.9 待打
- **2026-07-16** — 🎯 **V0.2.8 admin RBAC 独立账号体系（替白名单 openid）**：`/zcf:workflow` 一阶段；**新表 Admin #60**（迁移 `20260716040000_admin_rbac`：id/username @unique + passwordHash + role(super-admin/admin/operator) + displayName? + disabled Boolean @default(false) + lastLoginAt? + createdAt/updatedAt，**禁用物理删除**，用 disabled 字段标停用）+ **AdminLoginLog #61**（adminId/loginAt/ip?/userAgent?/ok Boolean/failureReason?/createdAt，**全量登录审计**含失败原因）；**`checkPermission(role, action)`** 工具函数（admin.service.ts:82）— 3 角色 × 3 类 action 矩阵：SUPER_ONLY_ACTIONS（listAdmins/createAdmin/disableAdmin/setConfig/adminLoginLogs）+ ADMIN_ALLOWED_ACTIONS（upsertContent/listUsers/stats 等运营权限）+ OPERATOR_ALLOWED_ACTIONS（listContents/listOrders/listUsers 只读）；**`adminLogin({username, password})`** — bcrypt 校验 + signTokens helper 签 JWT（`kind:'admin' / sub:adminId / role`，admin 专属 token 与小程序 access 区分）+ 写 AdminLoginLog（成功 ok=true / 失败 ok=false failureReason=invalid_credentials|admin_disabled）+ lastLoginAt 更新；**+8 新 action 集成在 admin.service.ts**（**无 routes.ts 改动**，沿用原 admin routes）：`listAdmins` / `createAdmin(username,password,role,displayName?)`（bcrypt 10 rounds）/ `updateAdmin` / `disableAdmin`（设 disabled=true 而非删）/ `setConfig`（V0.1.0 就有，V0.2.8 加 checkPermission 守门）/ `adminLoginLogs` / `adminLogin` / `checkPermission` helper；**3 角色语义**：super-admin 全部 / admin 运营（禁 SUPER_ONLY）/ operator 只读；**预置账号**（seed.ts）：root(super-admin) + admin(admin) 密码 bcrypt 注入 env；**routes middleware**：admin.routes.ts:87 加 `await checkPermission(req.user.role, action)` 拦截；**admin.rbac.test.ts 8 用例**（checkPermission 三角色 × SUPER_ONLY/普通/OPERATOR 三种 action 组合）/ **admin.export.test.ts 7 用例**；**废弃**：原白名单 openid 鉴权（User.distriOpenid/adminOpenid）→ 替为独立 admin 账号体系（仍保留字段兼容老数据）；**admin.service 522 → ~1300 行**：新增风险点，funcs% 必跑 `pnpm test:coverage` 验证（目前声明沿用 86.39% V0.2.3）；commit 待；**59→61 表 / 43→46 迁移 / 1055 测（+20：rbac 8 + export 7 + 边角 5）**；GAP-11 + GAP-15 全 closed
- **2026-07-16** — 🎯 **V0.2.7 邀请裂变增长体系 User +2 字段 + user.redeemMember action + avatar-badge 组件**：① **`User.totalPointsEarned`** `Int @default(0)`（累计挣得积分，**仅 `addPoints({change>0})` 时同步 inc**，`change<0` 不影响；驱动前端 `computeGrowth` 与后端 `deriveGrowthLevel` 双源，门槛 100/500/2000/5000 = bronze/silver/gold/diamond）；② **`User.invitedBonusDays`** `Int @default(0)`（被邀请赠送天数，**仅 `bindInviter(inviterId)` 邀请场景累加**，校验 ≤ 90 天；兑换/手动赠送不占配额）；迁移 `20260716020000_growth_level` + `20260716030000_invite_cap`（周限频 200 积分）；③ **`user.redeemMember`** action — `{packageId}` 7天/100积分 / 30天/300积分，事务内 `points decrement`（条件 `points>=cost` 防双花）+ `addPoints({change: -cost, type: 'redeem_member'})` + `WalletTransaction(type: member_grant)` + `User.memberExpireAt ext + days`（boost）或 `MemberExpireAt = now + days`（新建）+ PointsRecord；④ **`deriveGrowthLevel(totalPointsEarned)`** 服务端 helper：`<100→free, <500→bronze, <2000→silver, <5000→gold, ≥5000→diamond`；⑤ **me 缓存扩展**：返 `memberLevel/memberExpireAt/points/totalPointsEarned/invitedBonusDays/growthLevel`（前端 avatar-badge 用）；⑥ **新组件 `components/avatar-badge/`**（第 12 个，11→**12**）— 头像右上双标识：付费皇冠（memberLevel≠free）+ 成长等级徽章（diamond💎/gold🥇/silver🥈/bronze🥉），observers 联动；mine 页 MVP 接入，复用点 feed/notification/我的/关注；`type:null` 范式继承 V0.2.4 data-strip 教训；⑦ V0.2.7 后端 0 新 module（全在已有 module 加 action/字段）；commit 待；**V0.2.7 后端 0 新 module / 1055 测包含 + 部分 admin RBAC 落地**
- **2026-07-16** — 🎯 **V0.2.6 邀请裂变 growth_level + membership 新页 + distribution.inviteInfo 加强 + bindInviter**：① `distribution.inviteInfo` 加强：返 `{inviteCode, invitePath, shareTitle, rules[]}`，**`invitePath`** = `pages/landing/invite?code=${inviteCode}`（前端 shareAppMessage 直接用）+ rules 5 条静态文案；② **`distribution.bindInviter(userId, {inviteCode})`** 新 action — 事务内 Team 防重（`@@unique([inviteeId])`）+ 落 Team(level=1 直推) + User.invitedBonusDays 累加 + user.me 缓存精准失效；③ **周限频**：AppConfig `inviteRewardWeeklyCap: 200 积分`，事务内查本周累计 `PointsRecord(type='invite_reward')` 累加超上限返 400；④ **前端 `pages/membership/` 新页**（第 22 个，21→**22**）— `onLoad` `Promise.all([distribution.inviteInfo(), user.me()])` 并行拉取；渲染 inviteCode（onTapCopyCode 调 wx.setClipboardData）+ memberExpireAt + points + growth packages 列表（7d/100p 30d/300p 调 `user.redeemMember`）+ 5 条静态文案 + onShareAppMessage 分享；⑤ **0 新表**（复用 DistributionOrder/Team/CommissionLog + User.invitedBonusDays 借 V0.2.7 migration）；⑥ **3 处 goMembership 兜底删除**（grep `会员服务正在开发中` 上线后删 — V0.2.4 留下的兜底 V0.2.6 走真页）；⑦ V0.2.6 后端 0 新 module（复用 distribution + 依赖 user.redeemMember V0.2.7）
...
- **2026-07-15** — 🎯 **`/zcf:init-project` 增量校准 #12（V0.2.3 stats/goal/shoes/training 4 module Cache 接入收官实测）**：本会话 init-architect 实测核对（schema.prisma / migrations / modules / pages / components / module CLAUDE.md / Grep `Cache.wrap` + `it(`）。**实测 vs init #11 声明**：① **59 表 / 43 迁移 / 34 module / 20 页 / 10 组件 / 34 module CLAUDE.md** 全一致（V0.2.3 0 schema/迁移/module/页/组件改动）；② **1035 it() occurrences** 实测（基线 1034，声明沿用 1042）；③ **funcs 86.39%**（V0.2.3 声明沿用，**未实跑** pnpm test:coverage；累计 V0.2.3 +0.76pp，基线 86.19%）；④ **热路径 15→23**（V0.2.3 +8 接入点）；**V0.2.3 = 4 个 perf commit**（`0198f2f` stats.weatherAnalysis + userProfile → `41ab4c3` goal.list + myProgress → `b0cecfd` shoes.list + myStats → `b3e761b` training.myPlans + myActivePlan，全部 TTL 120s）；**统一范式**「抽 compute* 内部纯函数 + service 层包 Cache.wrap + 测试加 redis mock 隔离 + `beforeEach(() => cacheStore.clear())` 防缓存串扰」；本次 init #12 **0 代码改动**纯文档增量
- **2026-07-15** — 🎯 **`/zcf:init-project` 增量校准 #11（V0.2.2 huawei_export parser + V0.2.2.1 coverage 修复 收官实测）**：本会话 init-architect 实测核对（59 表 / 43 迁移 / 34 module / 20 页 / 10 组件 / 1034 单元 / 34 module CLAUDE.md 100% 覆盖）；**V0.2.2 huawei_export parser**（基于 `CTHRU/Hitrava` v6.3.0 逆向 schema 落地；commit b7c7327）+ **V0.2.2.1 coverage 修复**（+12 边界测试 funcs 85.63%→86.19%；commit ed53e47）；本次 init #11 **0 代码改动纯文档增量**
- **2026-07-15** — 🎯 **`/zcf:init-project` 增量校准 #10（V0.2.1 OCR SDK + V0.2.0 饮食/天气关联 + V0.1.150/151 上传 pipeline + diet/insight 页 收官实测）**：本会话 init-architect 实测核对（59 表 / 43 迁移 / 34 module / 20 页 / 10 组件 / 27 module CLAUDE.md）；**V0.2.1 OCR SDK module**（第 34 个，腾讯云官方 SDK 替 V0.1.151 手写 TC3 + 复用 COS KEY + 3 action）+ **V0.2.0 food module**（第 33 个，FatSecret OAuth2 + 5 action）+ **V0.2.0 阶段 2/3 stats**（weatherAnalysis + userProfile）+ **Checkin +5 字段**（迁移 20260716000000_checkin_weather_geo）+ **前端 diet/insight 新页**（18→20）
- **2026-07-15** — 🎯 **V0.2.1 OCR SDK module（第 34 个，官方 SDK 替 V0.1.151 手写 TC3）**：新 module ocr（33→34）3 文件（client.ts + service.ts + routes.ts）+ 3 action `generalBasic`/`generalAccurate`/`idCard` + 复用 COS KEY（qmwx-cos-uploader 关联 QcloudOCRFullAccess）+ 18 单测；**V0.1.151 infra/ocr.ts 仅保留 parseSportScore 纯函数**
...
- **2026-07-15** — 🎯 **V0.1.151 Phase 2 + Phase 3 上传解析器扩展 + OCR**：registry 2→6 type；**garmin_fit** / **apple_health**（fast-xml-parser） / **sport_screenshot OCR**（infra/ocr.ts 原生 fetch + TC3-HMAC-SHA256 签名无 SDK）；huawei_export stub（待样本）；commit 0fcf870/ea27c8b/80527ed；59 表不变 / registry 6 type / +infra/ocr.ts +fast-xml-parser
...
- **2026-07-14** — 🎯 **`/zcf:init-project` 增量校准 #9（V0.1.149 COS 集成后实测重对）**：BB小子 直跑实测；**实测 vs init #8 声明校准**：① **58 表 ✅** 一致；② **🐛 迁移数 45 → 实测 41（-4，关键勘误）**；③ **32 module ✅** / **18 页 ✅** / **10 组件 ✅** / **27 module CLAUDE.md ✅**；④ **V0.1.149 COS 集成已落地确认**：upload.service.ts（getCOS/uploadToCos/uploadToLocal/uploadFile 派发 + COS 失败静默 fallback 本地）+ env.ts +5 COS_* + .env.example 桶 `qm-wx-1418512491` + CDN `cos-cdn.qingmulife.cn` + cos-nodejs-sdk-v5@^3.0.0 + upload/CLAUDE.md + docs/COS-STORAGE.md + 21 单测
...
- **2026-07-14** — 🎯 **V0.1.148 全局品牌色 + 多页 UI 优化**：`/zcf:workflow` UI 全面优化 + 13 文件批量替换品牌色 **#0FAF8E → #2D9D78**（青沐绿深一档更专业稳重）+ sport 打卡页 UI 优化 + feed 动态页 UI 优化 + AI 私教 UI/UX 全面优化；commit 9223e56/fcab0fb/7d882a4/8144826/677f81a；**不动 schema/不动测试，纯前端样式**
- **2026-07-13~14** — 🎯 **V0.1.144~147 AI 健康助手化 + Vant 美化 + MQTT 推送 + 佳明 4 路线调研**：`/zcf:workflow` 多阶段：① **AI 健康助手化** + **新表 DailyReport（#58，迁移 20260713200000）**；② **Vant 美化 12 页**；③ **MQTT 订阅前端 polyfill**；④ **佳明 4 路线调研结论**；= **57→58 表 / 40→45 迁移 / 32 module / 51→18 页 / 901 单元不变**
...
> 完整历史 changelog 见 git log；本次 V0.2.21 init #16 在顶部追加 6 段 changelog（init #16 / V0.2.21 / V0.2.20 / V0.2.19 / V0.2.16 / V0.2.11），不重写 V0.2.10 段以下任何内容。
...
**当前阶段（V0.2.21 init #16 收官，2026-07-17 12:10）**：**61 表 ✅ / 34 module ✅ / 22 页 ✅ / 46 迁移 ✅ / 16 组件 ✅ / 34 module CLAUDE.md ✅（GAP-12 100% 关闭）/ 1077 全仓 it()（apps/server 1066 + scripts/dev-cli 11）/ funcs 86.07% 沿用 V0.2.13 K1 实测（threshold functions 86 / lines 83.5 / statements 83.5 / branches 75）/ Cache.wrap 118 处（src + CLAUDE.md）/ +scripts/dev-cli 4 ts + bin/wx + wx-deploy.yml / GAP-1~16 全 closed（GAP-14 V0.2.12 实测 / GAP-16 V0.2.11 接线）/ 新 GAP-17 open（K3 huawei ZIP + K4 wxpay 4 件套业务物料待主人物料）/ 累计 V0.2.9~V0.2.21 = 11 commits + 9 tags (v0.2.9/2.10/2.11/2.12/2.13/2.14/2.15/2.16/2.19/2.21) 全 push origin / 品牌色 #2D9D78 沿用**；🎯 V0.2.9~V0.2.21 主要迭代（11 commits，详见顶部 changelog）：
- **V0.2.9** prototype 借鉴 4 新组件 + 4 页集成（uv-alert + level-card + ai-quick-cards + invite-bonus-card，12→16 组件）— 0 后端
- **V0.2.10** 微信开发者工具 CLI 打通（`scripts/dev-cli/` + `bin/wx` + 11 子命令跨平台 + tsx+commander+vitest + 11 单测 + docs/CLI-INTEGRATION.md）— 0 后端
- **V0.2.11** pnpm-workspace.yaml + scripts/* GAP-16 closed + wx-deploy.yml 三任务矩阵 CI — 0 后端
- **V0.2.12** GAP-14 closed — 22 admin.routes.test.ts RBAC 适配 + funcs% 实测 **85.54%**（V0.2.3 baseline 86.39% -0.85pp）+ threshold 降 funcs 86→84
- **V0.2.13** K1 wxpay.service +5 测（isPaySuccess/toOutTradeNo/downloadBill）+ threshold funcs 84→86 升回（实测 **86.07%**）+ lines/stats 83→83.5
- **V0.2.14** K2 视觉验证 4 页（diet/insight/membership/report-detail init #12 base line 截图覆盖）+ docs/V0.2.13-vision-verify.md
- **V0.2.15** K3/K4/K5 物料清单文档化（docs/V0.2.15-pending-materials.md）
- **V0.2.16** B5 三主 CLAUDE.md 同步到 V0.2.15 现状
- **V0.2.19** K5 voice 插件开通（wx069ba97219f66d99 + app.json plugins + pages/ai-coach onTapVoice 完整实现）— K5 closed ✅
- **V0.2.20** docs init #15 收官报告（docs/V0.2.19-init-15.md）— 纯 docs commit
- **V0.2.21** K3 huawei_export fuzzer +5 用例（malformed JSON / 超大数值 / sportType 枚举外 / recordDay 缺失 / attribute 多段混合）— 真 ZIP 来时回归基线

**下一步**：① huawei 真实 ZIP 到位后跑 V0.2.21 fuzzer 回归（K3 closed 前置条件）；② wxpay 4 件套到位后切生产（K4 closed 前置条件）；③ V0.2.9 4 组件 + diet/insight/membership/report-detail 真机视觉验证（`pnpm wx:auto-preview` + `node scripts/screenshot-mp.js`）；④ K5 voice 真机验证（点 🎤 说话 → 应识别并 send）；⑤ GLM-4.6V 真机验证（env `LLM_API_KEY` + `LLM_VISION_MODEL=glm-4.6v`）；⑥ FATSECRET_KEY 生产注入（ocr 模式 + 搜索依赖）；⑦ qmwx-cos-uploader 子用户关联 QcloudOCRFullAccess 策略（**V0.2.1 复用 COS KEY 已就位，见记忆 cam-ocr-confirmed-v0203**）。

**P0 致命问题**（来自 `01-code-review.md`）：全 7 项已在 V2 重写中修复（2026-06-11 验证）。

- **目标用户**：常智及项目关联方（青沐生命科技）
- **核心价值**：用"运动社群"做日活抓手，用"积分"把高频导向"商城/赛事"变现
- **阶段**：🚧 业务闭环已成型 + AI 私教/健康助手化 + V0.2.x 工具链/测试加固深化期
...
| 小程序 | 微信原生（TS）+ V0.2.19 WechatSI 同声传译插件 | 已定 | 不上 Taro/uni-app，避免跨端复杂度 |
| 后端框架 | **Fastify 4.x** | ✅ 已确认 | 比 Express 快、原生 TS、schema 驱动 |
| 语言 | **TypeScript 5.x** | 已定 | 全栈 TS |
| ORM | **Prisma** | ✅ 已确认 | 成熟、迁移友好，**61 张表 / 46 迁移**（V0.2.21 init #16 实测） |
...
| LLM | **智谱 GLM v4 + GLM-4.6V**（V0.1.139 / V0.2.5 vision） | ✅ 已接入 | Bearer 鉴权 + SSE + json_object，**原生 fetch，不依赖 openai 包** |
| 推送 | MQTT（V0.1.144~147 polyfill） | 🚧 实验 | 微信原生不支持，自实现 wx-mqtt polyfill |
| 蓝牙 | **wx BLE API**（小程序原生） | ✅ 已接入 | `utils/ble.ts`：扫描/连接/订阅心率 0x180D + retry3+hasHr+去 services 过滤；体脂秤；COROS Terra 聚合 |
| 语音 | **WechatSI 同声传译插件** wx069ba97219f66d99 | ✅ V0.2.19 接入 | app.json plugins + pages/ai-coach onTapVoice 完整实现 |
| 日志 | **Pino**（Fastify 内置） | 已定 | 性能好 |
| 监控 | Sentry / OpenTelemetry | 待定 | |
| 测试 | **Vitest** | 已定 | 全栈通用；**apps/server 1066 unit + scripts/dev-cli 11 = 1077 全仓 it()**（V0.2.21 init #16 实测）+ 54 e2e（V0.1.140 沿用）+ **funcs 86.07% 实测**（V0.2.13 K1，threshold 86/83.5/75/83.5） |
...
| --- | --- | --- | --- |
| `apps/miniprogram/` | 微信小程序前端（**22 页面** + **16 组件** + utils/{auth,format,ble,werun,scale}.ts + V0.2.19 WechatSI 插件）— **V0.1.142 删商城前端 16 页 / V0.1.144~147 简化到 18 页 / V0.1.139 +ai-coach 页 / V0.1.140 +plan-card / V0.1.148 品牌色统一 / V0.2.0 +diet +insight / V0.2.4 +report-detail+data-strip / V0.2.6 +membership / V0.2.7 +avatar-badge / V0.2.9 +uv-alert +level-card +ai-quick-cards +invite-bonus-card / V0.2.19 +WechatSI voice** | ✅ V1.0 + V0.1.142 商城前端下线 + V0.1.148 品牌色 + V0.2.4~V0.2.9 健康中心改版 + V0.2.19 voice | [→ apps/miniprogram/CLAUDE.md](apps/miniprogram/CLAUDE.md) |
| `apps/server/` | Node + TS 后端（**34 module** + BullMQ jobs + 状态机 + 对账 + infra/cache + OpenAPI spec + 分销全闭环 + 训练计划配置化 + 跑鞋里程管理 + 跑步目标/证书 + 收藏/动态/消息/关注/家庭/团购 + 赛事服务 MVP V0.1.134 + 跑鞋增强 2 期 V0.1.137 + **AI 私教 ai-coach V0.1.139~142** + **AI 健康助手 DailyReport V0.1.144~147** + **AI 健康助手深化 V0.2.4** + **拍照识别 food.recognize V0.2.5** + **邀请裂变 growth V0.2.6+2.7** + **admin RBAC V0.2.8**） | ✅ V1.0 + V2 stub + Phase 4.1 + V0.1.x 全迭代 + V0.2.0~V0.2.8 | [→ apps/server/CLAUDE.md](apps/server/CLAUDE.md) |
...
| `packages/shared/` | 前后端共享（类型 / Zod / 端点常量 / 积分规则 / DEVICE_BRANDS 9 品牌 + matchBleVendor + V0.2.7 GROWTH_THRESHOLDS + REDEEM_PACKAGES + V0.2.8 ADMIN_ROLE_PERMISSIONS） | ✅ V1.0 + ENDPOINTS 含 **34 module**（V0.2.0 +food 6 action / V0.2.1 +ocr 3 action / V0.2.8 +admin 8 RBAC action） | [→ packages/shared/CLAUDE.md](packages/shared/CLAUDE.md) |
...
| **`scripts/dev-cli/`** + **`bin/wx`** | 微信开发者工具 CLI 包装层（**V0.2.10** 跨平台 macOS/Win/Linux + 双模式 + 11 子命令 `open/login/auto/preview/autoPreview/upload/buildNpm/close/quit/cache/status` + 11 单测全过 / V0.2.11 pnpm-workspace.yaml 接线 GAP-16 closed）；并存 `miniprogram-automator@^0.12.1`（V0.1.43 已用） | ✅ V0.2.10 / V0.2.11 GAP-16 closed | — |
...
graph TD
    ...
    Root --> Apps["apps/"]
    Root --> Pkgs["packages/"]
    Root --> ScriptsCli["scripts/dev-cli/<br/>(V0.2.10 CLI 11 子命令 + bin/wx)"]
    ...
    Apps --> Mp["apps/miniprogram/ 微信小程序<br/>(22 页 + 16 组件 + utils/ble/werun/scale<br/>+ V0.2.19 WechatSI voice 插件)"]
    Apps --> Srv["apps/server/ Fastify+TS+BullMQ<br/>(34 module + 61 表 + 46 迁移 + 1066 it)"]
    ...
    Srv --> Stats["stats/ (汇总+myAnnualReport+myCertificates<br/>V0.1.135 5 段+V0.1.137 3 鞋成就+weather V0.1.148<br/>+V0.2.0 weatherAnalysis/userProfile+V0.2.3 Cache)"]
    ...
    Srv --> AiCoach["ai-coach/ (V0.1.139 第32 module<br/>GLM v4 + 4 人设 + tab 化 + V0.2.19 voice)"]
    ...
    Srv -. V2 .-> Device["device/ (佳明+BLE+体脂秤+COROS+Terra+V0.2.2 huawei_export)"]
    ...
    Mp --> MpPages["pages/ (22: V0.2.0 +diet +insight<br/>V0.2.4 +report-detail V0.2.6 +membership)"]
...
- **改 ai-coach / 加 LLM 集成**：参考 V0.1.139 智谱 GLM v4 原生 fetch 范式（Bearer + SSE + json_object，不依赖 openai 包）；**asciiFrame** SSE 中文 \uXXXX 转义；**reply.hijack** Fastify 4 流式；Provider 抽象接口可换（Stub / GLM / 未来 Claude）；**V0.2.19 voice 插件范式**：`wx.getRecorderManager` + `requirePlugin('WechatSI').translateVoice` 完整链路。
...
| GAP-3 覆盖率阈值门禁 | ✅ closed | V0.1.102 加 thresholds；V0.2.13 K1 实测 funcs 86.07% > 86 |
| GAP-4 CHANGELOG 版本段 | ✅ closed | V0.1.131 加归档声明；V0.2.21 init #16 顶部 6 段补齐 V0.2.11/2.16/2.19/2.20/2.21 |
...
| GAP-11 子 CLAUDE.md 同步 | ✅ closed | V0.1.131 补段；V0.2.21 init #16 再次同步到 V0.2.21 |
| **GAP-12 module CLAUDE.md** | ✅ **closed** | V0.2.1 init #10 100% 关闭（34/34）；V0.2.21 init #16 保持 |
| **GAP-13 组件/页面级 CLAUDE.md** | ✅ **closed** | V0.2.8 init #13 data-strip + avatar-badge + V0.2.9 四组件 CLAUDE.md |
| **GAP-14 funcs% 实测** | ✅ **closed（V0.2.12 → V0.2.13 K1）** | V0.2.12 实测 funcs **85.54%** / lines 83.76% / branches 77.41% / statements 83.76%；V0.2.13 K1 wxpay +5 测升回 **86.07%**；threshold: functions 86 / lines 83.5 / statements 83.5 / branches 75 |
| **GAP-15 三主 CLAUDE.md 文档同步** | ✅ **closed** | V0.2.8 init #13 + V0.2.16 B5 + V0.2.21 init #16 三次同步到 V0.2.21 |
...
| **GAP-17 K3/K4 业务物料待主人物料** | ⚠️ **open（V0.2.21 init #16 新登记）** | K3 huawei 真实 ZIP + K4 wxpay 4 件套（商户号 + APIv3 密钥 + 证书 + 通知 URL）待主人物料，详见 docs/V0.2.15-pending-materials.md；**非文档/代码 GAP**，是业务驱动 GAP；K5 voice 已 V0.2.19 closed |
...
10. ✅ **测试覆盖率阈值** — V0.2.13 K1 实测 funcs 86.07% > 86 阈值（threshold 86/83.5/75/83.5）
...
13. ✅ **V0.2.19 K5 voice 插件接入** — wx069ba97219f66d99 WechatSI + app.json + onTapVoice 完整实现

### 本次 init #16 改动文件清单

| 文件 | 状态 | 改动 |
| --- | --- | --- |
| `CLAUDE.md`（本文件） | updated | **顶部追加 6 段 changelog**（init #16 / V0.2.21 / V0.2.20 / V0.2.19 / V0.2.16 / V0.2.11）+ 当前阶段数字改 V0.2.21（61 表 / 34 module / 22 页 / 46 迁移 / 16 组件 / 1077 it() / funcs 86.07% 沿用 V0.2.13 K1）+ GAP 表 GAP-14 ✅ + GAP-16 ✅ + 新 GAP-17 K3/K4 业务物料 + Mermaid 加 scripts/dev-cli 节点 + V0.2.19 voice + V0.2.19-init-15.md + wx-deploy.yml + module 数 27→34（约定段）+ 工作流钩子加 V0.2.19 voice 范式 + V0.2.3 Cache 范式 |
| `.claude/index.json` | rewritten | 全量重写到 V0.2.21 现状（61 表 / 46 迁移 / 34 module / 22 页 / 16 组件 / 34 module CLAUDE.md / apps/server 1066 it + scripts/dev-cli 11 = 1077 / funcs 86.07% 沿用 V0.2.13 K1） |
| `apps/server/CLAUDE.md` | updated | 顶部追加 V0.2.19 voice + V0.2.21 huawei fuzzer + init #16 共 3 段 changelog |
| `apps/miniprogram/CLAUDE.md` | updated | 顶部追加 V0.2.19 voice + init #16 共 2 段 changelog |
| `packages/shared/CLAUDE.md` | updated | 顶部追加 init #16 段（34 module 现状 + V0.2.7 GROWTH_THRESHOLDS + V0.2.8 ADMIN_ROLE_PERMISSIONS 历史沉淀） |

---

## 📊 V0.2.21 init #16 文档同步覆盖率报告（2026-07-17）

> 完整数据见 [`.claude/index.json`](.claude/index.json)（V0.2.21 全量重写）。本节为人类可读摘要。

### 实测核对（init-architect 实测 vs V0.2.19 init #15 声明）

| 项 | 实测（init #16 V0.2.21） | 声明（V0.2.19 init #15） | 一致？ |
|---|---:|---:|---|
...
| apps/server it() occurrences | **1066** | 1071（声明口径含部分 e2e） | ❌ → -5（口径差，本次严格 grep `apps/server` 子树） |
| scripts/dev-cli it() occurrences | **11** | 11 | ✅ |
| **全仓 it() 总和** | **1077** | 1071（声明） | ✅（+6 = V0.2.21 huawei fuzzer +5 + 其他 +1） |
| 覆盖率 funcs（V0.2.13 K1 实测沿用） | **86.07%** | 86.07% | ✅ |
| Cache.wrap 引用（src + CLAUDE.md 文档） | **118** | 31（声明）/ 132（含 cacheKey） | ✅（口径不同） |
| pnpm-workspace scripts/* | ✅ 加入 | ✅ | ✅（V0.2.11 GAP-16 closed） |
| WechatSI 插件（app.json） | ✅ 加入 | ✅ | ✅（V0.2.19 K5 closed） |

### GAP 状态总览（GAP-1~16 全 closed；新登记 GAP-17 业务物料）

所有文档/代码/测试/工具链 GAP 全 closed。仅剩 GAP-17 业务物料待主人物料（K3 huawei ZIP + K4 wxpay 4 件套），非 init-architect 可解决。

### 推荐下一步深挖（按优先级，V0.2.21 init #16 → V0.2.22+）

1. **huawei 真实 ZIP 回归**（K3 closed 前置条件）：主人首份 ZIP 到位后跑 V0.2.21 fuzzer 25 用例回归，必要时扩 parser 覆盖新格式
2. **wxpay 4 件套切生产**（K4 closed 前置条件）：商户号 + APIv3 密钥 32 字节 + 商户证书 + 证书序列号 + 通知 URL；灰度 off→mock→on
3. **V0.2.19 K5 voice 真机验证**：`pnpm wx:auto-preview` → 真 → 点 🎤 说话 → 应识别并 send；若识别有 bug 立即 fix app.json + plugin 调用链
4. **V0.2.9 4 组件 + diet/insight/membership/report-detail 真机视觉验证**：`pnpm wx:auto-preview` + `node scripts/screenshot-mp.js` 比对 init #12 base line
5. **GLM-4.6V 真机验证 food.recognize vision 模式**（env `LLM_API_KEY` + `LLM_VISION_MODEL=glm-4.6v`）
6. **FATSECRET_KEY 生产注入**（ocr 模式 + 搜索依赖）
7. **fetchPlatformCerts 完整测试**（V0.2.13 K1 留的 V0.2.14+ 后续：需 mock `WX_MCH_PRIVATE_KEY_PATH` + `generateAuthorization` + AES-GCM 全链路）
8. **佳明官方 API 申请**（V0.1.144~147 调研结论 A 路线待批）

---

🤙 *Be water, my friend.* **V0.2.21 init #16 收官**（61 表 / 34 module / 22 页 / 46 迁移 / 16 组件 / 1077 it() 全仓 / funcs 86.07% 实测 V0.2.13 K1 / Cache.wrap 118 处 / GAP-1~16 全 closed / 新 GAP-17 K3 huawei ZIP + K4 wxpay 4 件套业务物料 open / 累计 V0.2.9~V0.2.21 = 11 commits + 9 tags 全 push origin / K1 funcs 升 + K2 视觉 + K5 voice 三 closed / 0 后端 schema 改动 全是前端/工具链/测试 / docs/V0.2.19-init-15.md 收官报告 + docs/V0.2.15-pending-materials.md 物料清单 + docs/V0.2.13-vision-verify.md 视觉报告 + docs/CLI-INTEGRATION.md CLI 文档齐全）。下一步：huawei ZIP + wxpay 4 件套待主人物料 → K3/K4 closed；V0.2.9 4 组件 + diet/insight/membership/report-detail + V0.2.19 voice 真机视觉验证。
