...
**当前阶段（V0.2.140，2026-07-25 init #20 收官）**：**66 表 ✅ / 58 迁移 ✅ / 36 module ✅ / 36 module CLAUDE.md ✅（GAP-12 36/36）/ 29 页 ✅（实测 app.json）/ 17 组件 ✅ / 全仓 it() apps/server 1291→**~1380**（init #19 基线 + V0.2.119-129 +46 + V0.2.130-139 +13 + V0.2.140-153 +30 估算，**vitest 实跑未跑**）+ scripts/dev-cli **11** + packages/shared 6 = **~1397 全仓 it()** / **funcs 90.55% 实测**（lcov.info LF/LH=15128/13299 / FNF/FNH=646/585 → 90.55%，V0.2.79 90.64% 微跌 0.09pp 仍 > 86 阈值 4pp+ 🎯 健康）/ **lines 87.91%** / threshold 86/83.5/75/83.5 全过 / Cache.wrap src **118** 处 / **+ Flutter APP**（apps/flutter ~80 文件 Phase 1-3：4-tab + AI 流式 SSE + 跑群/跑鞋/里程碑/会员，Riverpod+go_router+M3）/ **V0.2.115-140 累计 25+ commits + 25 tags**（device 重构 + MQTT→WebSocket + 11 版 sprint + 10 版 sprint + 14 版 V0.3.0 入口）/ 品牌色 #2D9D78 沿用**；生产部署 **v0.2.87** healthy（2026-07-23 v0.2.47→v0.2.87 跨 40 版本全链升级 ✅）+ **v0.2.137** 生产已部署 healthy（V0.2.87→v0.2.137 跨 50 版，sshpass+scp+docker compose，DB schema 已由主人手动同步 5 个新迁移 deploy 时仅构建+重启，**关键修正**：docker-compose.yml POSTGRES_DB qmwx_dev→qmwx + DATABASE_URL 真实密码不入仓）/ ✅ GitHub 已同步 v0.2.57（远程 HEAD=0e7787d；v0.2.58-140 待 push）；🎯 V0.2.115-140 主要迭代（25 commits）：
...
**下一步**（V0.2.140 init #20 待办）：① **miniprogram_npm rebuild**（V0.2.131 + V0.2.141/143/148/150/152 共 6 处 shared 增量，下次部署前必 rebuild）；② **V0.2.66 提审**（mp 服务类目 + curl getCategory/uploadMedia/submitAudit）；③ **WechatSI 授权加回**（GAP-18，常智公众平台「插件管理」添加 wx069ba97219f66d99）；④ **V0.2.140+ V0.3.0 真机验证**（strength share/poster/suggest/overview/RPE/postHr/completionScore 6 个新功能 + 跑训结合总览 + VIVO 手环 UI）；⑤ **Mosquitto VIVO 蓝牙 Bridge 调研**（device V0.2.140+ 真接入待物）；⑥ wxpay 4 件套切生产（K4 GAP-17）；⑦ V0.2.33 interpret 真机验证（minimax key 注入 + 佳明 .fit 样本）；⑧ GitHub push V0.2.58~140 主线（HTTPS PAT 间歇重试，详见 memory [[github-ssh-key-push-pending]]）；⑨ vitest 实跑 funcs% 验证 + apps/server it() 实数（`pnpm -C apps/server test && pnpm -C apps/server test:coverage`，**init #20 估算 ~1380 实跑权威**）；⑩ huawei 真实 ZIP 回归（K3 GAP-17 closed 但待主人物料回归）；⑪ FATSECRET_KEY 生产注入；⑫ V0.2.116 MQTT→WebSocket GARMIN_CONSUMER_KEY 激活；⑬ qm-admin Web 跨仓同步 V0.2.140+ strength/Interpret 增强。
...
> 当前 tag：**`v0.2.140`**（**V0.2.140-153 V0.3.0 入口：device VIVO 手环 + 力量训练 V0.3 增量 + 跑训结合 14 成果 + DeviceDailyActivity #66** 2026-07-25 / **V0.2.130-139 10 版 sprint + 生产部署 V0.2.138**：shared 化 NotifType + 动作库用户自定义/收藏/趋势/历史 + docker-compose 修正 2026-07-25 / **V0.2.119-129 11 版 sprint**：realtime 单点集成 + strength 个人画像 + 力量计划闭环 2026-07-25 / **V0.2.115-117 device重构+MQTT→WebSocket+文案** 2026-07-24 / V0.2.94-113 小程序重构 + GPS 闭环 + 隐私协议 2026-07-24 / **V0.2.138 生产部署 v0.2.87→v0.2.137 跨 50 版 ✅** 2026-07-25 / V0.2.87 生产部署 v0.2.47→v0.2.87 跨 40 版 ✅ 2026-07-23 / init #20 V0.2.140 文档校准 2026-07-25 / init #19 V0.2.79 文档校准 2026-07-23 / V0.2.73-79 测试加固 +97 测 funcs 90.64%）；qm-admin 独立仓同步至 V0.2.37（Interpret.tsx 跨仓 commit f314235）；生产部署 **v0.2.137 healthy**（qingmulife.cn，2026-07-25 v0.2.87→v0.2.137 跨 50 版本全链升级 ✅）；CT400 Gitea `ct400` 保留不同步。**CHANGELOG.md** 已加归档声明（V0.1.131 起停更，完整 Changelog 主入口为根 CLAUDE.md 本段）。
