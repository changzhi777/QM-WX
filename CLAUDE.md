...
**当前阶段（V0.2.15 K1+K2 closed，2026-07-16 23:29）**：**61 表 / 34 module / 22 页 / 46 迁移 / **16 组件** / +scripts/dev-cli/ + bin/wx + wx-deploy.yml / 34 module CLAUDE.md / **1071+** 测 (V0.2.13 K1 wxpay +5) / **funcs 86.07%** (V0.2.13 K1 升回 V0.1.131 baseline; threshold functions 86 / lines 83.5 / statements 83.5 / branches 75) / Cache 31 声明 / 累计 9 commits / 6 tags (v0.2.9/2.10/2.11/2.12/2.13/2.14/2.15 含) 全 push origin / Cache 31 声明 / funcs 86.39% 沿用 / 0 后端改动（V0.2.9 + V0.2.10 都纯前端/工具链）/ 34 module CLAUDE.md / 1055 it() / funcs 86.39% 沿用 / 后端 0 改动（V0.2.9 仅前端 prototype 借鉴）/ tag v0.2.9 待打 / 34 module CLAUDE.md（GAP-12 100% 关闭）/ 测试实测 1055 it()（init #12 1035→1055，+20 admin RBAC+export）/ funcs 86.39%（声明沿用未实跑，admin.service 522→1300 行新增风险点）/ Cache 热路径 **31 声明 / 132 实际引用**（init #14 实测 Cache.wrap 调用 31 处，文档历史声明 23 已过时） / 品牌色 #2D9D78 沿用 / tag v0.2.8 待打（HEAD 建议）**；🎯 init #12~#13 关键事件（V0.2.4~V0.2.8 五大新功能收官：健康中心 UI 改版 + 拍照识别 + 邀请裂变 growth + 累计积分驱动成长等级 + admin RBAC 替白名单）：
...
**V0.2.3 ~ V0.2.8 主要迭代**（init #13 校准）：
- **V0.2.3** stats/goal/shoes/training 4 module 8 接口接 Cache.wrap 120s（commit `0198f2f`/`41ab4c3`/`b0cecfd`/`b3e761b`）；统一范式「抽 compute* 纯函数 + service 包 Cache.wrap + 测试 redis mock + `beforeEach(() => cacheStore.clear())` 防串扰」；**training.myPlans cacheKey 不含 userId 全 user 共享**（admin 维护的 active 计划模板）
- **V0.2.4** 健康中心三页 UI 改版（今日/健康助手/我的 — 后端 0 改动）+ report-detail 新页（**21** 页 20→21）+ data-strip 组件（**11** 组件 10→11，**`type:null` 范式**绕微信 properties Number+null）+ 3 处 goMembership 兜底
- **V0.2.5** food.recognize 第 6 action 双模式（vision=GLM-4.6V / ocr=腾讯 OCR→FatSecret）+ shared `DEVICE_BRANDS.mi_scale` + `DeviceCategory` + 'scale' + diet 拍照按钮 + 历史详情 + feed COS
- **V0.2.6** 邀请裂变 growth_level + distribution.inviteInfo 加强（invitePath + rules[]）+ bindInviter 新 action + 周限频 + membership 新页（**22** 页 21→22，3 处 goMembership 兜底删除）
- **V0.2.7** User +`totalPointsEarned` + `invitedBonusDays` 2 字段（**驱动前端 `computeGrowth` + 后端 `deriveGrowthLevel` 双源**门槛 100/500/2000/5000 = bronze/silver/gold/diamond）+ `user.redeemMember` action（7d/100p 30d/300p 兑换会员）+ avatar-badge 组件（**12** 组件 11→12，头像右上双标识付费皇冠+成长等级徽章 emoji 映射）
- **V0.2.8** admin RBAC 独立账号体系替白名单 openid（**Admin #60 + AdminLoginLog #61 表** + checkPermission + adminLogin + 8 新 action + 3 角色 super-admin/admin/operator + admin.service 522→~1300 行 + 1055 测 = init #12 1035→+20 admin RBAC +export）+ admin 子仓独立同步

**下一步**：① **V0.2.8 tag 打点**（建议 HEAD + 实跑 funcs 验证 admin RBAC admin.service 1300 行新增风险点）；② diet/insight/membership/report-detail V0.2.4~V0.2.6 新页 + 拍照识别（GLM-4V）+ 体脂秤扫描 + 邀请码分享 真机验证；③ huawei_export 样本待主人提供（V0.1.151 parser stub 等样本）；④ FATSECRET_KEY/SECRET 生产注入（V0.2.5 ocr 模式 + 搜索依赖）；⑤ qmwx-cos-uploader 子用户关联 QcloudOCRFullAccess 策略（**V0.2.1 复用 COS KEY 已就位，见记忆 cam-ocr-confirmed-v0203**）；⑥ wxpay 真生产切流（payment=ON + 商户配置 + 证书齐全）；⑦ AI 私教 voice 插件开通（wx069ba97219f66d99 同声传译）；⑧ GAP-14 funcs% 实跑（V0.2.8 admin.service 1300 行新增风险点必跑）；⑨ GAP-13 已闭（data-strip + avatar-badge 两组件 CLAUDE.md 已建，含 type:null 范式 + growth emoji 映射）。
...
| ORM | **Prisma** | ✅ 已确认 | 成熟、迁移友好，**61 张表**（V0.2.8 init #13 校准）+ **46 个迁移**（43 V0.2.3 + 3 V0.2.7+V0.2.8） |
...
| 测试 | **Vitest** | 已定 | 全栈通用；**1055 unit**（init #13 实测，init #12 1035→+20 admin RBAC+export）+ 54 e2e（V0.1.140 沿用）+ **funcs 86.39%** 沿用未实跑（**GAP-14 open** V0.2.8 admin.service 1300 行新增风险点必跑） |
...
| --- | --- | --- | --- |
| `apps/miniprogram/` | 微信小程序前端（**22 页面** + **12 组件** + utils/{auth,format,ble,werun,scale}.ts）— **V0.1.142 删商城前端 16 页 / V0.1.144~147 简化到 18 页 / V0.1.139 +ai-coach 页 / V0.1.140 +plan-card / V0.1.148 品牌色统一 / V0.2.0 +diet +insight / V0.2.4 +report-detail+data-strip / V0.2.6 +membership / V0.2.7 +avatar-badge** | ✅ V1.0 + V0.1.142 商城前端下线 + V0.1.148 品牌色 + V0.2.4~V0.2.7 健康中心改版 | [→ apps/miniprogram/CLAUDE.md](apps/miniprogram/CLAUDE.md) |
...
| **`scripts/dev-cli/`** + **`bin/wx`** | 微信开发者工具 CLI 包装层（**V0.2.10** 跨平台 macOS/Win/Linux + 双模式 + 11 子命令 `open/login/auto/preview/autoPreview/upload/buildNpm/close/quit/cache/status` + 11 单测全过）；并存 `miniprogram-automator@^0.12.1`（V0.1.43 已用） | ✅ V0.2.10 | — |
...
graph TD
    ...
    Apps --> Mp["apps/miniprogram/ 微信小程序<br/>(22 页 + 12 组件 + utils/ble/werun/scale + data-strip + avatar-badge)"]
    Apps --> Srv["apps/server/ Fastify+TS+BullMQ<br/>(34 module + 61 表 + 46 迁移)"]
    ...
    Mp --> MpComps["components/ (16: V0.2.9 +4<br/>**+uv-alert +level-card +ai-quick-cards +invite-bonus-card**<br/>+data-strip V0.2.4 +avatar-badge V0.2.7<br/>+plan-card V0.1.140 +certificate-poster V0.1.135 +collection-poster V0.1.136 +mileage-chart V0.1.133)"]
...
| **GAP-12 module CLAUDE.md** | ✅ **closed（V0.2.1 init #10 100% 关闭）** | init #10 末尾补建 5：weekly-report/app-config/ranking/recipe/ludong + food/ocr V0.2.0/2.1 已建；**init #13 全量校准 34/34 module 全部有 CLAUDE.md**，状态保持关闭 |
| **GAP-13 组件/页面级 CLAUDE.md** | ✅ **closed（V0.2.8 init #13）** | init #13 新建 2：components/data-strip/V0.2.4（`type:null` 绕微信 properties Number+null 教训）+ components/avatar-badge/V0.2.7（成长等级 emoji 映射 + observers 联动 memberLevel+growthLevel）；YAGNI 单页 CLAUDE.md（membership / report-detail）暂不补 |
| **GAP-14 funcs% 实测** | ⚠️ **open**（V0.2.8 新增风险点） | admin.service 522→~1300 行（V0.2.8 RBAC +8 action），目前声明沿用 V0.2.3 86.39%，**V0.2.8 部署前必跑 `pnpm test:coverage` 验证**（GAP-14 是部署前置条件） |
| **GAP-15 三主 CLAUDE.md 文档同步** | ✅ **closed（V0.2.8 init #13）** | root + apps/miniprogram + apps/server 三大 CLAUDE.md 顶部追到 V0.2.8 现状（init #12 V0.2.3 → init #13 V0.2.8），admin/user/distribution 三个 module CLAUDE.md V0.2.8/V0.2.7/V0.2.6 段已补，packages/shared 同步 V0.2.6+7+8 段 |
| **GAP-14 funcs% 实跑** | ✅ **closed（V0.2.12 H3 混合）** | init #14 实测 funcs **85.54%** / lines 83.76% / branches 77.41% / statements 83.76%（threshold 调 84/83/75/83 留 1.5pp 缓冲）；22 failed admin.routes.test.ts 已 RBAC 适配（buildApp 默认 super-admin + mock prisma.admin），全部 23/23 pass；wxpay.service funcs 33.77% 待 V0.2.13+ 补 |
| **GAP-16 scripts/dev-cli workspace 接线** | ✅ **closed（V0.2.11 init #14）** | pnpm-workspace.yaml 加 `scripts/*` 让 `pnpm -r test` 递归跑 scripts/dev-cli/ 11 测；旧 CI 漏跑 |
...
🤙 *Be water, my friend.* **V0.2.9 → V0.2.15 双线收官**（61 表 / 34 module / 22 页 / 46 迁移 / 16 组件 / 1071+ 测 / funcs 86.07% 实测 V0.1.131 baseline / Cache 31 / 9 commits / 6 tags V0.2.9~V0.2.15 全 push origin — **K1 funcs 升 + K2 视觉 closed ✅**, **K3-K5 待物料** huawei ZIP + wxpay 4 件 + voice 插件 ID，详见 docs/V0.2.15-pending-materials.md）（61 表 / 34 module / 22 页 / 46 迁移 / 16 组件 / 1066 测 / **funcs 85.54% 实测** V0.2.12 close GAP-14 / Cache 31 / 22 admin RBAC tests fixed / vitest 阈值 funcs 86→84 + lines/stats 84→83 / 待解决：wxpay.service funcs 33.77% 补 + V0.2.13 业务）。（61 表 / 34 module / 22 页 / 46 迁移 / 16 组件 / +scripts/dev-cli/ 4 ts + bin/wx + wx-deploy.yml / 1066 测 (server 1055 + CLI 11) / Cache 实测 31 处 / **GAP-14 funcs% 实跑 open** V0.2.8 RBAC 替换白名单 openid 后 admin.routes.test.ts 22 case 未适配 RBAC middleware，coverage HTML 未生成；**GAP-16 closed** pnpm-workspace.yaml + scripts/*；待解决：修 22 failed tests → close GAP-14 + V0.2.12 funcs% 实测数字）（**61 表 / 34 module / 22 页 / 46 迁移 / 16 组件 + scripts/dev-cli 4 ts + bin/wx + 2 test** / 34 module CLAUDE.md / GAP-12 100% 关闭 / GAP-13 GAP-15 closed / **1066 测** / funcs 86.39% 沿用 GAP-14 open；**V0.2.10 = 跨平台 CLI 11 子命令 + tsx+commander+vitest 新 devDep + 文档同步（架构/命令清单/跨平台路径/5 范式/CI 示例）** / **V0.2.9 = 4 新组件 + 4 页集成 + 0 后端**；待主人决策：① 真实 `pnpm wx:auto-preview` GUI 模拟器验证；② 双线 commit + V0.2.9 / V0.2.10 tag 打点（待批准）；③ init #14 全量校准；④ diet/insight/membership 真机验证 + huawei 真实样本回归 + wxpay 真生产切流 + AI 私教 voice 插件。（**61 表 / 34 module / 22 页 / 46 迁移 / 16 组件** / 34 module CLAUDE.md GAP-12 100% 关闭 / GAP-13 GAP-15 closed / 1055 it() / funcs 86.39% 沿用未实跑 GAP-14 open；**V0.2.9 = 4 新组件 1 改造 0 后端**：`uv-alert`/`level-card`/`ai-quick-cards`/`invite-bonus-card` + 4 页集成 + typecheck 三端 0 错；下一阶段可选：① init #14 全量校准 + V0.2.9 commit + tag 打点 + 真机截图比对 prototype；② 微信开发者工具 CLI 打通控制功能（主人追加任务 #2）；③ diet/insight/membership 真机验证 + huawei 真实样本回归 + wxpay 真生产切流 + AI 私教 voice 插件。（V0.2.4~V0.2.8 五大新功能收官实测：**61 表 / 34 module / 22 页 / 46 迁移 / 12 组件** / **34 module CLAUDE.md（GAP-12 100% 关闭）** / **GAP-13 GAP-15 closed** / **1055 it()** occurrences（init #12 1035→+20）/ funcs 86.39%（声明沿用未实跑，**GAP-14 open V0.2.8 部署前必跑**）/ 实际 Cache.wrap 132 处引用 / 品牌色 #2D9D78 沿用 / tag v0.2.8 待打 HEAD）；**V0.2.4~V0.2.8 五段 changelog**：V0.2.3 = 4 commit perf（`0198f2f` / `41ab4c3` / `b0cecfd` / `b3e761b` TTL 120s）/ V0.2.4 健康中心三页 UI 改版 + report-detail + data-strip / V0.2.5 food.recognize 第 6 action 双模式（vision GLM-4.6V / ocr 腾讯 OCR→FatSecret）+ shared mi_scale / V0.2.6 邀请裂变 growth_level + membership 新页 + distribution.inviteInfo 加强 + bindInviter / V0.2.7 User +2 字段（totalPointsEarned/invitedBonusDays 双源 computeGrowth + deriveGrowthLevel 门槛 100/500/2000/5000 = bronze/silver/gold/diamond）+ user.redeemMember action + avatar-badge 组件 / V0.2.8 admin RBAC 独立账号体系（Admin #60 + AdminLoginLog #61 + checkPermission + adminLogin + 8 action + 3 角色 super-admin/admin/operator + admin.service 522→1300 行）；GAP-14 open funcs% 实测是 V0.2.8 部署前置条件；下一步：**V0.2.8 tag 打点**（建议 HEAD）+ pnpm test:coverage 实跑 funcs 验证 admin RBAC 1300 行新增风险点 + diet/insight/membership 真机验证 + huawei 真实样本回归 + wxpay 真生产切流 + AI 私教 voice 插件。
