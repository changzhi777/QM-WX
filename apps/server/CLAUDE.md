...
> ## 📋 变更记录 (Changelog)
>
> - **2026-07-25 (V0.2.119-129 11 版 sprint：realtime 业务事件 + strength 个人画像 + 力量计划闭环)** — 🎯 **apps/server 11 成果（V0.2.116 wx 通道后真正消费阶段）**：① **V0.2.119 notification realtime 单点集成**：`notification.service.ts` notify() 写库后 `try { publishToUser(userId, 'notification', {type, targetType, targetId, content, actorId}) }` — feed.like/comment/follow 全部走 realtime；前端 mine/feed 页订阅 toast/红点（`onRealtime('notification', cb)`）；顺手修 V0.2.115 device.bindings mock 缺口（`heartRateRecord.findFirst`）；② **V0.2.120 strength 力量训练小程序前端（3 页）**：apps/miniprogram 训记式 index/session/detail 三页（**后端 V0.2.42 已就绪，无后端改动**）；③ **V0.2.121 goal 成就 realtime 推送（距离目标）**：`goal.service.ts` +`detectAndMarkJustAchieved(userId, todayDistance, todayDateStr)` 算法 `before = aggregate - todayDistance; after = aggregate; justAchieved = before < target && after >= target` + period 跨日跳过 + status='completed' 防重；`sport.service.checkin` step 6 集成 + `notifyGoalAchieved(userId, {id, title, kind:'distance', target: targetDistance})` — `🎯 目标「XXX」已达成！100km`；④ **V0.2.122 strength 完成 realtime 推送**：`strength.service.finishSession` 后调 `notifyStrengthDone(userId, {id, totalVolume, setCount})` — 自触发（user 是 actor）不跳 self-skip，新建函数不复用 notify()；`💪 训练完成！本次 2400 kg·次 · 3 组`；⑤ **V0.2.123 strength 组间休息倒计时**（UX 升级，无后端改动，schema 已有 restSec 字段）；⑥ **V0.2.124 strength 训练目标（新维度）**：Prisma **Goal +`targetVolume Float?`**（**新迁移 `20260725000000_goal_strength_volume`**，targetDistance 改 @default(0） + `goal.schema.ts` +`GoalKindEnum = z.enum(['distance','volume'])` + `kind` 字段 + service.kind-aware（targetVolume!=null → volume 走 StrengthSession.aggregate dateStr 范围） + `detectAndMarkStrengthJustAchieved`（复用 V0.2.121 范式） + strength.finishSession 集成 + notifyGoalAchieved `kind:'volume'` 显示 `kg·次`；⑦ **V0.2.125 feed 粉丝 realtime 推送**：`feed.service.publish` 后查 followers + `Promise.allSettled(notify({type:'new_post', targetType:'feed', targetId: feed.id}))` 隔离单点失败；⑧ **V0.2.126 strength 详情页增强**：`strength.service.ts` +`getExerciseStats(userId)` — PB（最大重量，并列 reps 多优先）+ 容量分布（按 totalVolume 降序 + percent，precision `Math.round(*1000)/10`）；frontend +🏆 PB 卡 +📊 分布柱状；⑨ **V0.2.127 strength 主页训练日历热图**（纯前端 UX，**0 后端改动**）；⑩ **V0.2.128 力量训练计划（产品新维度）**：Prisma **TrainingPlan +`kind String @default("running")`**（**新迁移 `20260725001000_training_plan_kind`**） + seed 2 力量计划（strength_beginner 12×3 / strength_intermediate 16×4，targetKm=0 兼容） + `training.service.myPlans({kind?})` 支持过滤 + cacheKey 拼 `:kind` + `calcPlanProgress` kind-aware + frontend training 页力量卡 `plan-card--strength` 渐变 + 💪 角标；⑪ **V0.2.129 力量计划完成通知（生命周期闭环）**：Prisma **TrainingPlan +`targetSessions Int?`**（**新迁移 `20260725002000_training_plan_target_sessions`**） + seed 力量计划带 targetSessions（12×3=36 / 16×4=64） + `training.service.detectAndMarkPlanCompleted(userId)` 复用跨阈值范式（before=sessionCount-1, after=sessionCount） + `notifyPlanCompleted(userId, {id, name, targetSessions})` `🎉 力量计划「XXX」已全部完成！共 36 次训练` + strength.finishSession 一次触发 3 个独立检测（容量目标 + 力量目标 + 计划完成），每个 try/catch 隔离；**notification.schema.ts** NOTIF_TYPES 加 `plan_completed`（共 8 类事件）；**关键范式沉淀**：① **realtime 单点集成**：notify() 写库后内部 `try { publishToUser }` — 业务侧零感知；② **跨阈值精准检测**：before/after 双 sample + period 跳过 + status 防重（V0.2.121→V0.2.129 4 次复用）；③ **跨 module 集成**：strength.finishSession 一次触发 3 个独立检测（每个 try/catch 隔离），不破坏主链路；④ **kind 隐式判定**：targetVolume!=null 即 volume 目标，0 schema 破坏；⑤ **strengthSession 复用**：训练计划进度 / 容量目标 / 计划完成 都从 StrengthSession.aggregate 拿数据，单一数据源；**测试**：**apps/server 1291 → 1337** it + **+46** 测（V0.2.119-129 11 版本累积：goal/kind/strength/session/plan 检测全覆盖）；**schema 改动**：Goal +targetVolume + kind / TrainingPlan +kind + targetSessions（**3 个新迁移**：20260725000000/01000/02000）；**待办**：生产部署 V0.2.87 → V0.2.129 跨 42 版本（含 3 新迁移需 `prisma migrate deploy`）

- **2026-07-23 (init #19 V0.2.79 收官)** — 🎯 **`/zcf:init-project` 增量校准 #19**：本会话 init-architect 阶段 A/B/C 全量实测 + 主智能体 vitest 实跑 cross-check（**apps/server it() = 1291 passed + 62 skipped = 1353 total / 117 test files** `pnpm -C apps/server test` vitest 实跑权威数字；声明沿用 1277 已订正为 1291）/ scripts/dev-cli 16 / packages/shared 6 / **全仓 1313 合计** / **funcs 90.64% / lines 87.77% / branches 79.28%**（threshold 86/83.5/75/83.5 全过缓冲 4pp+）/ **65 表 / 49 迁移 / 36 module / 25 页 / 16 组件** / **GAP-12 36/36 closed**（V0.2.42 strength/CLAUDE.md 新建 + apps/flutter/CLAUDE.md 新建）/ Cache.wrap 118 处 / typecheck 三端 exit 0 / 0 schema/迁移/module 改动；**V0.2.40-79 重大迭代覆盖**：① **V0.2.42 strength**（第 36 module，训记式力量训练 7 action + StrengthSession/Set/Exercise #63-65 + 迁移 20260720000000_strength + Exercise seed ~15 + volume 实时累加 + cnDate CN 时区 + ai-coach context-builder V0.2.46 聚合）；② **V0.2.45 ai-coach 多模态识图 + 深上下文**（ChatMessage.content 扩 `string|ContentPart[]` + extractText/hasImage + GLM-4.6V + HISTORY_TURNS 10→20 + context-builder 加 Meal/StrengthSession 7 天）；③ **V0.2.47 huawei_export TCX**（GAP-17 K3 closed parseTcxXml + 1633 真实回归 13982km）；④ **V0.2.49 admin funcs 加固**（6 未测函数全补 + RBAC 审查无越权漏洞 / admin.module 76.59→89.36%）；⑤ **V0.2.50-57 funcs 90% 三连** + **interpret screenshot GLM-4.6V 闭环**（V0.2.57 stage 5 stub 补全）；⑥ **V0.2.60 interpret P1 用户确认 checkin + 去重 + 限流 30/分 + 隐私**（+extract+checkinConfirmedAt 迁移 20260721000000 / interpret 32→40 测）；⑦ **V0.2.63-66 H5 fallback + 提审 API**（/h5/ 静态 + Redis 5min token + myInterpretHistory + mp getMpCategory/uploadMpMedia/submitMpAudit SUPER_ONLY + interpret 39→45 测）；⑧ **V0.2.67-72 Flutter APP**（详见 apps/flutter/CLAUDE.md，独立仓 ~80 文件）；⑨ **V0.2.73-79 +97 测**（GAP-3.5 routes 7 module：feed 嵌套 + strength 0→9 + device 2→30 + stats+6 + goal+4 + shoes+4 + content+4 + service 层 5 module：user.routes 5 + points 3 + wxpay 8 + review 2 + review/sport 6 + training.compute* 3 + food.recognize 7 + funcs 列序认知修正）；**主智能体 6 处修正根 CLAUDE.md**（略，详见根 CLAUDE.md init #19 段）+ **Mermaid 全块重写**（apps/flutter 节点 + Strength 节点 + apps/server 描述更新）；**重写** strength/CLAUDE.md（顶部 changelog 0→21 测）+ **重写** interpret/CLAUDE.md（顶部 changelog 32→45 测 + 新增 4 action + 范式点 9-12）；**0 代码改动纯文档增量**；下一步：① 部署 v0.2.57 interpret screenshot 到生产（LLM_API_KEY+LLM_VISION_MODEL 注入）；② 提审发布 v0.2.66（mp 服务类目申请 + curl 一键提审）；③ cross-check apps/server it() -14 异差已通过 vitest 实跑关闭（1277→1291 是声明与 vitest 实跑差 14 是 commit message 沿用旧值）；④ wxpay 4 件套切生产（K4 GAP-17）；⑤ V0.2.60-61 真机验证截图→识别→确认打卡流程；⑥ strength 前端 pages/strength 落地 + 组间计时 UI

- **2026-07-21** — 🎯 **V0.2.63-66 screenshot H5 fallback + 提审 API（后端）**：① **V0.2.63** interpret +issueH5Token/verifyH5Token（Redis 5min token）+ myInterpretHistory + routes `/h5`+`/h5/checkin`（token 鉴权 multipart→COS→interpretScreenshot）+ @fastify/static `/h5/`（interpret.html 单页）；② **V0.2.64** auth.ts 加 `/h5/`+`/uploads/` 静态前缀跳过 JWT（@fastify/static 路由无 config.public 被拦 401 修复）；③ **V0.2.65-66** `infra/wx-token.ts` getMpAccessToken（cgi-bin/token Redis 7000s）+ admin getMpCategory/uploadMpMedia/submitMpAudit（mp API 转发，SUPER_ONLY 独占）；**0 迁移** / +6 测（interpret 39→45）/ admin 89 测无回归 / typecheck exit 0 / 生产部署 healthy
> - **2026-07-21** — 🎯 **V0.2.60 interpret screenshot P1 加固（用户确认 checkin + 去重传图 + 限流 + 隐私）**：审查 13 项发现 → 修 6 项 P1：① P1.1 client +`callGlm` 文本版（第二次分析不传图省 ~50% token）/ P1.2 service 重构 `interpretScreenshot`（不 auto checkin，返 extract）+ 新 `confirmScreenshotCheckin`（查 record.extract + 去重 + checkin + 标 checkinConfirmedAt）+ routes +screenshotCheckin case + rateLimit 30/分；② P1.3 EXTRACT_PROMPT +date 字段 + P1.4 去重（Checkin findFirst userId+date+distance+dataSource='sport_screenshot'）+ P1.6 前端隐私提示；③ **1 迁移** InterpretRecord +extract Json? +checkinConfirmedAt DateTime?（20260721000000）/ +8 测（interpret 32→40，含 confirmScreenshotCheckin 4 测）/ 0 回归（1187 passed）；生产部署 49 migrations up to date；commit 87a2366 + push；V0.2.61 endpoints +interpret.screenshotCheckin 补（前端调用映射）
> - **2026-07-21** — 🎯 **V0.2.52-57 funcs 90% 达标 + interpret screenshot 多模态识图闭环**：① **V0.2.52/53 docs GAP-15 同步**；② **V0.2.51/54/55/56 funcs 90% 三连补测**：strength 12 测（0→100%）+ queue +5（57→92.85%）+ user.repository +11（50→100%）+ logger +4（66→100%）→ **全局 funcs 86.63→90.01% 达 90 目标 🎯**（threshold 86/83.5/83.5/75 缓冲 4pp）；③ **V0.2.57 interpret screenshot 阶段 5 stub 补全**：`client.ts` +`callGlmVision`（GLM-4.6V OpenAI vision，Bearer+ContentPart[]+response_format，复用 food.recognize 范式）+ `service.ts` +`interpretScreenshot`（①识图提结构化 JSON ②distanceKm>0→sportService.checkin dataSource='sport_screenshot' ③buildUserContext 13 路画像联动 ④GLM 综合分析 ⑤落 InterpretRecord）+ `routes.ts` screenshot case（按 action 分开 minimax/GLM 守卫）+ context-builder export buildUserContext + ENDPOINTS +interpret.screenshot + 前端 pages/interpret +📷 截图入口 + **+12 测**（interpret 20→32）/ **0 新表/迁移** / 复用 GLM-4.6V + context-builder + sportService.checkin（device pipeline 一致数据源）；**apps/server it() 1136→1180**（+44）/ funcs 86.63→**90.01%**（+3.38pp）/ 36 module / 65 表 / 49 迁移 不变；commit `0e7787d` + tag `v0.2.57` + GitHub 已 push（main + 12 tag v0.2.46-57）
> - **2026-07-21** — 🎯 **V0.2.40-51 后端大迭代（weather/strength/ai-coach 多模态+深上下文/huawei TCX/admin 加固/strength 补测）**：① **V0.2.40-41** stats weather icon 999→emoji（weatherIconToEmoji）+ weatherAir action 实现（修「999 长沙」）；② **V0.2.42 strength module（第 36）** 训记式力量日志（7 action + StrengthSession/StrengthSet/Exercise #63-65，迁移 20260720000000 + Exercise seed ~15）；③ **V0.2.45 ai-coach 多模态识图（GLM-4.6V）+ 更深上下文**（ChatMessage.content 扩 string|ContentPart[] + extractText/hasImage + HISTORY_TURNS 10→20 + context-builder 加今日饮食 Meal/近 7 天力量 StrengthSession）；④ **V0.2.47 huawei_export TCX 支持（GAP-17 K3 closed）** parseTcxXml（fast-xml-parser）+ parseHuaweiExport fallback + 1633 真实活动回归（13982km）；⑤ **V0.2.49 admin funcs 加固** 6 未测函数 +12 测 + RBAC 审查（无越权）；⑥ **V0.2.51 strength 补测** 12 测（0 测修复）；**全局 funcs 86.63→89%**（超 88 目标，strength 0→100% 是关键）/ 61→**65 表** / 47→**49 迁移** / 34→**36 module** / apps/server it() 1108→**1136**（+28）/ GAP-17 K3 closed + GAP-12 36/36（strength/CLAUDE.md V0.2.48 补建）
