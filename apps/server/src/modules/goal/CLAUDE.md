...
- **2026-07-28 (V0.3.7-16 健康目标闭环 + Phase 6 recommendGoals 系统推荐)** — 🎯 **goal module V0.3.x 大幅扩展**（5 个版本增量，**1 schema 改动**）：① **V0.3.7 Phase 3 健康目标闭环**：`Goal +5 字段`（迁移 `20260728000000_goal_health_kinds`：kind/targetValue/currentValue/unit/judgeCriteria）+ `calcHealthGoalProgress` 6 类 health kind（distance/volume/weight_loss/sleep/quit_smoking/mood）+ `computeGoalProgress` 统一入口（kind 分流：distance/volume 老路径 vs health 新路径）+ `updateProgress` action（手动更新 mood/sugar/dampness）；② **V0.3.8 Phase 4 友好失败**：`computeGoalProgress` 加 expired 检测 + suggestedTarget 降难度建议（"💡 未完成没关系，建议调整目标到 X 继续努力"）；③ **V0.3.9 Phase 5 暂停/恢复**：`pauseGoal`（active→paused）/ `resumeGoal`（paused→active）2 action（清单"可暂停"要求）；④ **V0.3.12 ENDPOINTS.goal hotfix**：补 `updateProgress/pauseGoal/resumeGoal` 3 action 显式登记（按 V0.2.61 范式）；⑤ **V0.3.16 Phase 6 recommendGoals 系统推荐**：`computeRecommend(userId)` 8 规则画像引擎（newcomer/low_distance/mid_distance/high_distance/low_volume/high_volume/weight_loss/sleep_low + priority 排序 + 排除已有同类 active goal + 5 类数据源 Promise.all 并行：User.createdAt + Checkin month agg + StrengthSession month agg + BodyCompositionRecord latest + SleepRecord 30d avg）；**测试**：`goal.recommend.test.ts` 9 测（V0.3.16 8 规则全测 + 排除 active goal）；**关键范式**：① **kind 分流统一入口**（computeGoalProgress 老路径 vs health 新路径，V0.3.7 范式）；② **跨阈值精准检测复用**（V0.2.121→V0.2.129→V0.3.16 recommendGoals 同源）；③ **画像规则引擎**（5 类数据源 Promise.all 并行 + priority 排序，与 dashboard/globalSearch 范式同源）；**前端配合**：`pages/goal/` 新建 4 文件（V0.3.7）+ 🎯 系统推荐卡片（V0.3.17 绿渐变 + 优先级角标 + 一键添加）+ expired 友好失败 UI + 暂停/恢复按钮
