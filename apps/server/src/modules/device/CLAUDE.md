...
- **2026-07-24** — 🎯 **V0.2.89-91 Garmin Health OAuth A 路线直连骨架（3 commit，与 D 路线 Terra 并存）**：① **V0.2.89 OAuth 1.0a 骨架**：garmin-health.ts 补全 HMAC-SHA1 签名（手动 RFC 5849，不装库）+ request_token/access_token（3-legged）+ push webhook 解析骨架；infra/crypto.ts 新增 AES-256-GCM encryptToken/decryptToken（存 OAuth token）；env +CRYPTO_KEY；device.service +garminHealthAuthUrl/garminHealthCallback；device.routes +garminHealthAuthUrl action + /garmin-health-callback public GET；9 测；② **V0.2.90 授权闭环深化**：garminHealthAuthUrl Redis 暂存 requestToken/secret（garmin:oauth:req:{userId} 5min TTL）+ garminHealthCallback Redis 取 + state=userId + DeviceBinding upsert（vendor=garmin_oauth 区分 BLE 加密 token/secret）+ del；11 测；③ **V0.2.91 webhook 全 5 type 落库**：garminHealthWebhook userId 映射（DeviceBinding vendorUserId）+ activities→RawActivity upsert（SI *InSeconds/*InMeters + activityType 映射 trail_running→running）+ sleep→GarminSleep upsert（秒映射）+ health(dailies)+stress→GarminMetric create + body_composition→BodyCompositionRecord create（g→kg）+ 单条异常不阻塞 + num() helper；14 测 + typecheck 0；⚠️ **Garmin push schema 字段名（*InSeconds/*InMeters/*InGrams）为推测，1B 佳明文档校准**；BodyCompositionRecord 无 @@unique → create（1B 加 @@unique [userId,timestamp,source]）；GarminSleep create as never cast（Prisma 联合类型）；**1B 待激活**：主人 GARMIN_CONSUMER_KEY/SECRET（佳明批 V0.1.144）+ 佳明 push JSON schema 文档 → 填端点 env + 校准 schema + 真测；commit 87ca9a4(v89)/32cb217(v90)/65bcfc1(v91) + tag v0.2.89/90/91 push GitHub；**与 D 路线（Terra V0.1.146 generateGarminAuthUrl）并存**：A 直连免费（OAuth 1.0a push）/ D Terra 收费但框架就绪（OAuth 2.0 webhook）
