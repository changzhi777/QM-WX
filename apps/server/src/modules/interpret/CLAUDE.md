...
- **2026-07-27** — 🎯 **V0.3.2 interpret 错误一致性 + 2 错误响应测**（commit 7159fcf）：interpret module 内 `throw new Error` → `throw Errors.*`（featureDisabled/notFound/badRequest 走 setErrorHandler 返统一 4xx/5xx 结构，避免低层错误信息泄露给前端）；**2 错误响应测**新增（验证 Errors.featureDisabled/notFound 走 setErrorHandler 返正确 4xx/5xx 结构）；**0 schema/迁移/module** 改动；与 V0.3.3 admin 错误一致性（admin MIS sprint 第 1 步）+ V0.2.156 strength 错误一致性（getSessionReport/getCompletionScore `throw new Error` → `throw Errors.notFound`）同范式 — **统一错误响应范式沉淀**（throw Error → throw Errors.* 是项目级 KISS 一致性提升，避免 try/catch 漏接导致低层错误 500 给前端）；**测试**：interpret 45 → 47 测（+2 错误响应）；**待办**：interpret 47 测 vitest 实跑待校