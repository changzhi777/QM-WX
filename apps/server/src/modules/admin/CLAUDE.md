...
- **2026-07-29 (V0.3.34 sprint A admin MIS 增强 8 子项)** — 🎯 **admin module V0.3.34 大幅扩展**（sprint A 8 子项，**0 schema/迁移改动**，纯 admin action 增强 + 1 依赖 exceljs + Redis pub/sub 远程热更新）：① **A2 getUserDetail(userId) 5 维聚合**（`admin.service.ts`：user 基本信息 + training 30 天 + orders 总数+paid+营收分 + points 当前+最近 10 条流水 + auditLogs 最近 10 条；Promise.allSettled 失败隔离 V0.3.4 dashboard 范式；改 distanceKm → distance Checkin model 字段名；删 lastLoginAt User 不含字段）；② **A3 batchUpdateOrderStatus + batchRefundOrder**（orderIds 1-100 数组 + 每单独立处理 + 部分失败不影响其他 + 返 success/failed/totalSuccess/totalFailed）；③ **A4 globalSearch 增强**（`admin.globalSearch.ts`：GlobalSearchInputSchema 加 types[]（user/feed/comment/interpret/strength 5 枚举）+ startDate + endDate；5 表查询加 type gate + date filter 增量，user 表无 createdAt 索引跳过 date filter）；④ **A5 dashboard dailyTrend 图表**（`admin.service.ts:getAdminDashboard()` 加 dailyTrend 字段 30 天每日 orders/newUsers/checkins；3 个 prisma.$queryRaw 聚合 + mergeDailyTrend helper 内存合并）；⑤ **A6 exportOrdersExcel exceljs**（`admin.service.ts:exportOrdersExcel(input)` 返 {filename, base64}；11 列：订单ID/openid/昵称/手机/状态/总金额/实付/积分/商品数/创建时间/支付时间；限制 5w 行防 OOM；依赖 exceljs ^4.4.0；exportUsersExcel/exportSettlementExcel 留待后续）；⑥ **A7 listAuditLogs targetType**（ListAuditLogsSchema 加 targetType 字段；`target.startsWith(\`${targetType}:\`)` 前缀匹配避免 'user:abc' 误匹配 'userfake'）；⑦ **A8 config 远程热更新**（`infra/realtime.ts` 加 publishFeatureFlagsUpdated + subscribeFeatureFlags；feature-gate onReady 钩子订阅 → 收到事件清 _cache；setConfig 改 feature_flags 时广播 'system:feature_flags_updated' channel；所有 worker 实例同步生效无需重启）；**关键范式沉淀**：① **batch 操作 Promise.allSettled 失败隔离**（A3 — 返 success/failed 列表，每单独立事务，部分失败不影响其他）；② **startsWith 前缀匹配避免 contains 误匹配**（A7 — 'user:abc' 不会误匹配 'userfake'）；③ **Redis pub/sub fire-and-forget void 模式**（A8 — publish 失败不阻塞 setConfig 响应，60s TTL 兜底）；④ **queryRaw 内存合并 vs 单 queryRaw + JOIN**（A5 — 3 个 roundtrip vs 1 个，可优化 v0.3.35+）；**0 schema/迁移/module 改动**；**测试**：admin module +5 测（A2-A8 routes/service 测试，apps/server test files 133→138）；**跨仓**：qm-admin 独立仓同步 3 前端 commit（A1 Dashboard 30s poll + A2 Users Drawer + A5 Recharts + A6 Excel 按钮）+ 1 测试 commit（91→142 测）；**待办**：① A6 exportUsersExcel / exportSettlementExcel 待补（commit 范围外，下个 sprint）；② A5 mergeDailyTrend 单 queryRaw + LEFT JOIN 优化；③ A3 batch 事务优化；④ admin module funcs% 实跑验证（V0.3.34 新增 ~20 函数）
...
- **2026-07-27 (V0.3.3-5 admin MIS sprint：错误一致性 + dashboard + globalSearch)** — 🎯 **3 成果（admin 后台能力增强 sprint 第 1-3 步）**：① **V0.3.3 admin 错误一致性 + 2 测**（MIS sprint 第 1 步）：admin module 内 `throw new Error` → `throw Errors.*`（forbidden/badRequest/notFound 走 setErrorHandler 返统一 4xx/5xx 结构，避免低层错误信息泄露给前端）+ 2 错误响应测；② **V0.3.4 admin.dashboard 1 API 拉全**（MIS sprint 第 2 步）：`admin.service.ts:1396-1471` 新增 `getAdminDashboard()` + `AdminDashboardData` interface（**9 字段**：totalUsers / activeUsers7d / totalOrders / totalRevenueFen 元→分 `*100+Math.round` / paidOrders / totalCheckins / checkins30d / failedAdminLogins30d / totalInterpret）+ 8 个 prisma 聚合并行 `Promise.allSettled` 失败隔离（user.count×2 + order.count + order.aggregate + checkin.count×2 + adminLoginLog.count + interpretRecord.count，任一失败返 0 dashboard 不整体崩）+ `admin.schema.ts:304-315` 加 `AdminDashboardResponseSchema` + `admin.routes.ts:140-143` 加 `case 'dashboard'`（admin + super-admin 可访问，operator 已被 middleware 拦截）；③ **V0.3.5 admin.globalSearch 5 表 LIKE 跨表**（MIS sprint 第 3 步）：**新建独立文件 `admin.globalSearch.ts`（~85 行）**（关键设计：admin.service.ts 已 1472 行，单文件拆分控制单文件复杂度）；5 表 LIKE 跨表搜索 — user（nickname insensitive + phone + openid）+ feed（content）+ feedComment（content）+ interpretRecord（result）+ strengthSession（notes），5 prisma 查询并行 `Promise.allSettled` 失败隔离（V0.3.1 cron pull 范式复用，任一失败返空数组）+ `admin.schema.ts:317+` 加 `GlobalSearchInputSchema`（query 1-50 字符 + limit 1-20 默认 5）+ `GlobalSearchResponseSchema` + `admin.routes.ts:144-148` 加 `case 'globalSearch'`；**关键范式沉淀**：① **Promise.allSettled 失败隔离复用**（dashboard/globalSearch/cron pull 三处都用，部分失败不影响整体响应）；② **dashboard/globalSearch 沿用 admin generic action 模式**（shared ENDPOINTS.admin 不单独登记，与 V0.2.37 listInterpret 同模式）；③ **单文件拆分控制复杂度**（admin.service.ts 1472 行 + admin.globalSearch.ts 85 行独立文件）；**0 schema/迁移**（admin.dashboard/globalSearch 沿用 admin generic action 模式，shared ENDPOINTS 不加）；**测试**：admin module 测试 +4（错误一致性 2 + dashboard/globalSearch routes 测试，本轮新增 — vitest 实跑待校）/ funcs% 未实跑（admin.service 1472 行新增 9 字段 dashboard + globalSearch 5 prisma 调用，建议下轮实跑 `pnpm -C apps/server test:coverage` 验证 admin module funcs 拉低幅度）/ **GAP-12 36/36 保持** / admin 文件清单 `admin.routes.ts + admin.service.ts + admin.schema.ts + admin.globalSearch.ts + admin.rbac.test.ts + admin.export.test.ts + admin.service.test.ts`；**待办**：① admin funcs 加固（dashboard/globalSearch 错误分支测试薄弱 — prisma 失败时 Promise.allSettled 兜底）；② V0.3.6+ admin MIS sprint 第 4 步方向待主人决定（用户管理增强 / 订单管理增强 / 内容运营 / 数据可视化等）
