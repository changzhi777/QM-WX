...
- **2026-07-16** — 🎯 **V0.2.6 邀请裂变增长体系 + membership 页接入**：① `inviteInfo` action 加强：返 `{inviteCode, invitePath, shareTitle, rules[]}`，**`invitePath`** = `pages/landing/invite?code=${inviteCode}` 小程序页面路径（前端 shareAppMessage 直接用）；**`rules[]`** 5 条静态文案（最高 20% 佣金/0 元开店/三级分销/秒到账/专属客服）；② **`distribution.bindInviter(userId, {inviteCode})`** 新 action — 在用户被邀请流程中（membership 页 / onboarding）调，事务内查 `User.findUnique({inviteCode})` 找上线；防自邀（inviterId !== userId）+ 防重（Team 表 @@unique(inviteeId)）+ 落 Team(level=1 直推) + User.invitedBonusDays 累加；缓存精准失效 user.me；③ **周限频**：AppConfig `inviteRewardWeeklyCap: 200 积分`（每周最多领 200 积分），事务内查询本周累计 `PointsRecord(type='invite_reward')` 累加，超上限返 400；④ **0 新表**（复用 DistributionOrder/Team/CommissionLog，team.inviterId 已能表达所有邀请关系；User.invitedBonusDays 字段借 V0.2.7 migration）；⑤ **前端 `pages/membership/` 新页**：`onLoad` `Promise.all([distribution.inviteInfo(), user.me()])` 并行拉取，渲染 inviteCode（onTapCopyCode 调 wx.setClipboardData）+ memberExpireAt + points + growth packages 列表（`REDEEM_PACKAGES`：7d/100p 30d/300p 调 `user.redeemMember`）+ 5 条静态分销文案 + onShareAppMessage 分享 inviteCode+shareTitle+invitePath；⑥ **依赖 V0.2.7 User.redeemMember**：本 action 仅暴露 inviteInfo 接口，redeeMember 走 user module（关注点分离）；⑦ **3 处 goMembership 兜底弹"会员服务正在开发中"**：grep 上线后删兜底（V0.2.4 留下的兜底文案 V0.2.6 已移除走真页）；本次 V0.2.6 后端 0 新 module（复用 distribution + 依赖 user.redeemMember）
