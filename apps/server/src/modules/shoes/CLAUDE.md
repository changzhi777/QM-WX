# shoes module — 我的跑鞋（V0.1.133 增强 + V0.1.137 对比 + 鞋评关联）
...
> 引入版本：**V0.1.26**（2026-07-03，pic 2768 跑者向）+ **V0.1.133**（阈值个性化 + 历史里程曲线 + 详情页）+ **V0.1.136**（Shoe +feeds relation）+ **V0.1.137**（compareShoes 横向对比）
> 相关 pic：2768（跑鞋里程管理）
...
**跑鞋管理**：CRUD + 健康度计算（currentKm / thresholdKm * 100）+ 退役 + 统计 + **V0.1.133 详情/历史/阈值** + **V0.1.137 横向对比**。
...
| `shoes.service.ts` | 9 action（V0.1.26 5 + V0.1.133 3 + V0.1.137 1）+ 导出 `incrementShoeKm` 供 sport 复用 | ~280 |
...
| **`getMileageHistory`**（V0.1.133） | `{ id }` | `MileageHistory` | 历史里程曲线（weekly + monthly 双粒度，garmin cm→km 分流） |
| **`updateThreshold`**（V0.1.133） | `{ id, thresholdKm }` | `{ id, thresholdKm }` | 单字段原子更新阈值（100-2000） |
| **`compareShoes`**（V0.1.137） | `{ ids: [string, string] }` | `{ left: ShoeCompare, right: ShoeCompare, winner: { field: 'left' \| 'right' \| 'tie' }[] }` | **横向对比 2 双跑鞋**（含 checkinCount 批量 groupBy + daysSincePurchase + healthRatio + 当前里程）；返胜出项字段（前端高亮） |
...
```bash
# tests/modules/shoes/shoes.service.test.ts — **16 单元**（V0.1.133 +9）+ V0.1.137 +N（compareShoes）
# tests/modules/shoes/shoes.routes.test.ts — 7 单元 + V0.1.137 +1 case
pnpm test shoes
```

覆盖：list 排序 / add / update 校验存在 / retire active→retired / retire 重复退役 badRequest / myStats retiringSoonCount / **getDetail（正常 + notFound）/ getMileageHistory（garmin cm→km 单位分流 + sport km 直通 + 双粒度分桶 + 空数据 + notFound）/ updateThreshold（正常 + notFound）/ incrementShoeKm 纯函数（事务客户端调用）/ V0.1.137 compareShoes（双鞋正常 + notFound + 同一双 badRequest）**。
