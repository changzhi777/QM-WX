...
- **2026-07-28 (V0.3.11 buildAlertText 三规则 + DailyReport.alertText)** — 🎯 **stats module V0.3.11 健康提示卡**：新增 `buildAlertText(steps, restingHr, sleepHours)` 函数 — 3 规则画像引擎：① sleep<6h → 「昨晚睡眠仅 X 小时，建议避免高强度运动，以拉伸+散步为主」；② hr>90 → 「静息心率 X bpm 偏高，建议今日低强度运动」；③ steps<3000 → 「今日步数仅 X 步，建议增加活动量」；④ 否则返回 null（无 alert）；**集成**：`dailyReport` action 写库 alertText（`stats.service.ts:271 const alertText = buildAlertText(steps, restingHr, sleepHours)`）；**架构图 v2.0 #3**（pages/index AI 快报卡是架构图 v2.0 增量第 3 项）；**前端配合**：`pages/index` AI 快报卡显示 alertText（V0.3.11）；**测试**：`stats.daily-report.test.ts` 5 测（含 alertText 3 规则分支）；**关键范式**：**规则引擎 + 降级 null**（3 规则均不触发时返 null，前端 wx:if 守门不显示卡片，V0.3.11 范式）
