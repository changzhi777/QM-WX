...
> ## 📋 变更记录 (Changelog)
>
> - **2026-07-29 (V0.3.35 miniprogram_npm rebuild — ENDPOINTS.boohee 7 action 前端可用)** — 🎯 **apps/miniprogram V0.3.35 零代码改动**（后端 boohee module 对接，前端 0 改动）：① **miniprogram_npm rebuild 已执行**（`node scripts/build-mp-shared.mjs` → 10 文件更新含 `endpoints.js` boohee 7 action + `feature-flags.js` boohee flag）；② **ENDPOINTS.boohee** 前端可通过 `import { ENDPOINTS } from '@qm-wx/shared'` 引用（search/detail/categories/categoryFoods/foodUnits/batchNutrition/foodRanking）；③ **feature-gate `boohee`** 默认 false（前端入口隐藏，后端远程开启后显示）；④ **实际前端页面待建**（boohee module 是后端 API 对接，前端 pages/food 或新 pages/boohee 消费待后续 sprint）；**实测**：31 页 / 18 组件不变（0 新页 0 新组件）；**待办**：① 前端 pages 消费 boohee API（食物搜索页 / 详情页，后续 sprint）；② feature flag boohee 远程开启（部署后 admin setConfig）
>
> - **2026-07-29 (init #23 V0.3.34 校准 + V0.3.25-29 UI 去重 sprint)** — 🎯 **apps/miniprogram V0.3.25-34 增量**（10 tags，**0 新页 / 0 新组件 / 0 新 utils / 0 新 services**，全是 UI 去重 + 缓存修复 + 提审准备 + hallmark 持续优化）：① **V0.3.25 hallmark UI 简化**（auth-center-section + device-auth + goal wxml/wxss 持续优化，commit `60c728e`）；② **V0.3.27 UI 去重**（删 AI 健康提醒 + AI 主动提醒卡 + 设备页品牌宫格融合到 auth-center-section，commit `e460176`）；③ **V0.3.28 dailyReport 缓存修复**（步数显示 0 bug → 缓存命中时实时更新 steps + reportText + alertText + healthScore，commit `9bf120a`；**后端 stats.service.ts 改动见 apps/server 段**）；④ **V0.3.29 UI 精简**（隐藏今日页 3 数据卡 + mine 页健康助手入口 + 设备页体脂秤入口恢复 P0 + 死代码清理 onTapBrand/onUnbind + 残留样式 -66 行 + mine 数据授权管理跳转 bug 修复，commit `d6e5ec8`）；⑤ **V0.3.29 提审准备**（getLocation 接口申请审核通过 + 隐私指引完善 12 字段全填 + van-button 修复 @vant/weapp 72 组件复制到 miniprogram_npm）；⑥ **V0.3.30-34 apps/miniprogram 0 改动**（V0.3.30-33 qm-admin 独立仓 + V0.3.34 后端 admin sprint A，不动小程序前端）；**实测**：**31 页 ✅ / 18 组件 ✅**（全部与 init #22 一致，V0.3.25-34 0 新页 0 新组件）/ V0.3.x 0 新 utils / 0 新 services；**WechatSI 插件状态保持 V0.2.25 临时移除**（GAP-18 open 待主人授权）；**miniprogram_npm rebuild 必要性**：V0.3.25-34 shared ENDPOINTS 0 改动（V0.3.34 admin sprint A 是后端 generic action 模式，shared 不加 ENDPOINTS），**本次 init #23 无需 rebuild**（但 V0.3.9+16 ENDPOINTS.goal +4 action 的 rebuild 待办从 init #22 延续）；**GAP-12 36/36 保持**（apps/miniprogram 0 新 module）；**待办**：① miniprogram_npm rebuild（ENDPOINTS.goal +4 action 待办延续，下次部署前必 rebuild）；② V0.3.29 提审推进（mp 后台手动提审，submitAudit 86000 API 仅第三方）；③ V0.3.25-29 真机验证（UI 去重 + dailyReport 缓存修复 + hallmark 持续优化）；④ WechatSI 授权加回恢复 K5 voice（GAP-18）；⑤ 未提交 wxml/wxss 处理（V0.3.24-29 hallmark UI redesign 持续优化）
>
> - **2026-07-28 (init #22 V0.3.24 校准收官 + 31 页/18 组件 + hallmark UI redesign)** — 🎯 **apps/miniprogram V0.3.6-24 增量**（6 项前端改动 + 1 新组件 + 3 新页 + 1 删页）：① **V0.3.6 Phase 1 入口收敛**：`pages/more` **物理删**（Glob 已确认 No files found）+ mine 3 入口 flags 守门（feature_flags 远程开关 + wx:if 守门不删代码）+ 饮食卡隐藏 + `feature_flags +3 diet/shoes/runner`；② **V0.3.7 Phase 2-3 保留页简化 + 健康目标闭环**：sport/health/device/interpret wxml 注释包裹（git 可恢复）+ **`pages/goal/` 新建 4 文件**（健康目标闭环，配合后端 Goal +5 字段 + calcHealthGoalProgress 6 类 health kind）；③ **V0.3.10 `pages/about/` 新建 4 文件**（架构图 v2.0 #4 关于沐禾健康：顶部品牌 logo + brandName + version + 核心功能介绍 + 关于我们 + footer 公司名）；④ **V0.3.11 pages/index AI 快报卡**（DailyReport.alertText 显示 sleep<6/hr>90/steps<3000 三规则提示）；⑤ **V0.3.17 pages/goal 🎯 系统推荐卡片**（绿渐变 + 优先级角标 P{N} + 一键添加按钮 + V0.3.8 expired 友好失败 + V0.3.9 暂停/恢复按钮）；⑥ **V0.3.20 `pages/device-auth/` 新建 4 文件 + `components/auth-center-section/` 新建 4 文件**（设备授权中心：品牌头部 + 状态徽标卡 + 最近 5 条活动列表 + web-view OAuth 跳转 mask + 章节组件 brands 列表渲染 + 操作按钮）；⑦ **V0.3.24 hallmark UI redesign**（auth-center-section + device-auth + goal wxml/wxss 持续优化；最新 commit `e9d1e13` 简化 brand-card class 不用嵌套三元 + 未提交 6 文件 wxml/wxss）；**实测**：**31 页 ✅**（V0.3.6 删 more -1 + V0.3.7 加 goal +1 + V0.3.10 加 about +1 + V0.3.20 加 device-auth +1 = 净 +2 vs init #21 29）/ **18 组件 ✅**（V0.3.20 +auth-center-section）/ V0.3.x 0 新 utils / 0 新 services；**WechatSI 插件状态保持 V0.2.25 临时移除**（GAP-18 open 待主人授权）；**miniprogram_npm rebuild 必要性**：ENDPOINTS.goal V0.3.9 加 updateProgress/pauseGoal/resumeGoal 3 action + V0.3.16 加 recommend 1 action = **+4 action**，下次部署前**必 rebuild**；**GAP-12 36/36 保持**（apps/miniprogram 0 新 module）；**待办**：① miniprogram_npm rebuild（ENDPOINTS.goal +4 action）；② V0.3.24 提审准备（`/Applications/wechatwebdevtools.app/Contents/MacOS/cli upload --project $PWD/apps/miniprogram --version V0.3.24`）；③ V0.2.66 提审推进（mp 服务类目 + curl getCategory/uploadMedia/submitAudit）；④ V0.3.6-24 真机验证（pages/goal 系统推荐卡片 + 暂停/恢复 + pages/about + pages/device-auth + auth-center-section + ai-coach 会员分层免费用户每日3次 + pages/index AI 快报卡 alertText）；⑤ WechatSI 授权加回恢复 K5 voice（GAP-18）；⑥ 未提交 6 文件 wxml/wxss 处理（V0.3.24 hallmark UI redesign 持续优化）
>
> - **2026-07-27 (init #21 V0.3.5：apps/miniprogram 0 代码改动声明)** — 🎯 **V0.3.1-5 期间 apps/miniprogram 源代码 0 改动**（V0.3.x 全是 apps/server admin MIS sprint + interpret 错误一致性 + device-poll-pull.job.ts 多 vendor 骨架，不动小程序前端）；**实测**：29 页 ✅ / 17 组件 ✅ / V0.3.x 0 新页 / 0 新组件 / 0 改动；**待办**：① miniprogram_npm rebuild（V0.2.131+141/143/148/150/152 共 6 处 shared 增量，下次部署前必 rebuild）；② V0.2.66 提审（audit-mp.sh ready，需 mp 后台加服务类目）；③ WechatSI 授权加回恢复 K5 voice（GAP-18，常智公众平台「插件管理」添加 wx069ba97219f66d99）；④ V0.2.140+ V0.3.0 真机验证（strength share/poster/suggest/overview/RPE/postHr/completionScore 6 个新功能 + 跑训结合总览 + VIVO 手环 UI）；⑤ 未提交 2 个 docs 文件待主人决定 git add（`docs/沐禾健康小程序UI功能清单.xlsx` + `docs/沐禾健康小程序功能思维导图.html`）；**子 CLAUDE.md 落后段补齐说明**：apps/miniprogram/CLAUDE.md 顶部落后根 CLAUDE.md 三段（V0.2.130-153 + V0.3.1-5），本轮 init #21 只 prepend 本声明段，V0.2.130-153 历史段不补（避免单次 init 改动过大；详情见根 CLAUDE.md V0.2.130-153 段 + V0.2.140-153 段）

> - **2026-07-25 (V0.2.119-129 11 版 sprint：realtime 消费 + strength 完整闭环)** — 🎯 **apps/miniprogram 9 成果（V0.2.116 wx 通道 + V0.2.119-122 realtime 上游齐备 → 下游前端消费阶段）**：① **V0.2.119 notification realtime 订阅**：`pages/mine/index.ts` + `pages/feed/index.ts` onShow 调 `connectRealtime() + onRealtime('notification', cb)` + onUnload `clearRealtime('notification')` + NOTIF_TYPE_TEXT + onRealtimeNotification toast 文案按 type 智能切换（like/comment/follow/goal_achieved/strength_done/new_post/plan_completed 共 8 类事件）；② **V0.2.120 strength 力量训练小程序前端（3 页闭环）**：新建 `pages/strength/{index, session, detail}` + app.json +3 路径（25→28 页）；index（容量概览 + 7 日柱状 + 历史列表 + 开始训练入口）/ session（自动计时 + 实时容量累加 + 动作 picker + 备注 + 完成返回）/ detail（按动作分组的组明细 + 备注）；③ **V0.2.122 strength 完成 realtime toast**（前端零改动，V0.2.119 mine 订阅自动接收 strength_done 文案 `💪 训练完成`）；④ **V0.2.123 strength 组间休息倒计时（UX 升级）**：`pages/strength/session.ts` +restTimer 状态机（addSet 后启动 90s 倒计时 + ±15 调整 + 到 0 `wx.vibrateShort({type:'medium'})` 触觉 + 切换 ready 态（渐变 + pulse 动画）+ 跳过不计入 restSec） + 透传实际 restSec 给后端；⑤ **V0.2.124 goal add 弹层加 kind 切换**：`pages/runner/index.{ts,wxml}` 加 `kind: 'distance' | 'volume'` 选择（🏃 距离 km / 💪 容量 kg·次） + 智能切换输入框 + placeholder + 目标卡展示按 kind 智能显示单位；⑥ **V0.2.125 feed 粉丝 fan-out**（前端零改动，mine 自动接收 new_post 文案 `关注的人发了新动态`）；⑦ **V0.2.126 strength 详情页增强（PB + 容量分布）**：`pages/strength/detail.{ts,wxml,wxss}` 并行拉 `sessionDetail + getExerciseStats` + 🏆 PB 卡（按 maxWeight 降序，并列 reps 多优先）+ 📊 容量分布柱状（5 级颜色：0/1-25/25-50/50-75/75-100%）；⑧ **V0.2.127 strength 主页训练日历热图（GitHub 风格）**：`pages/strength/index.{ts,wxml,wxss}` +`buildHeatmap(trend)` 函数（26 周 × 7 天 + Sunday 对齐 + 5 级颜色映射）+ 复用 myVolume(180) 同源数据 + 横向 scroll-view + 点格跳详情或 toast 显示「日期 + 容量」+ meta 文字「共 X 次训练 · 最高单日 X kg·次」；⑨ **V0.2.128 力量训练计划**：`pages/training/index.{ts,wxml,wxss}` loadPlans 并行拉 `Promise.all([myPlans({kind:'running'}), myPlans({kind:'strength'})])` + `plan-card--strength` 修饰（白底 + 绿边渐变 + 💪 力量角标）；**0 新组件 / 0 新页面**（仅 train 主页和 strength 三页改），**28 页 / 16 组件**（V0.2.120 起的 3 页 strength）；**前端订阅零回归**：mine/feed 页 toast 自动按 realtime type 智能切换（V0.2.119-122 上游齐备 → V0.2.124-129 下游消费）；**关键设计沉淀**：① **事件订阅零认知**：mine/feed 页只 1 处 `onRealtime('notification', cb)`，新增 type 不需要前端订阅点改动，只在文案 map 加一行；② **UX 反馈统一**：realtime 文案按 type 给「有人赞/评论/关注/目标达成/训练完成/新动态/计划完成」差异化 toast + 红点 +1；③ **复用胜过新写**：strength 热图复用 myVolume 180 天数据切片，0 新接口；**miniprogram_npm 沿用 V0.2.42 rebuild**（V0.2.119-129 shared 仅 +1 ENDPOINTS（strength.getExerciseStats，V0.2.126） + plan endpoints（V0.2.128） 0 改，miniprogram_npm 无需 rebuild）；**待办**：① 部署生产 V0.2.87→V0.2.129 跨 42 版本；② miniprogram_npm rebuild（如 shared 又有新增，**本次无**）；③ V0.2.66 提审 + WechatSI 授权（GAP-18）

- **2026-07-23 (init #19 V0.2.79 收官)** — 🎯 **`/zcf:init-project` 增量校准 #19**：本会话 init-architect 全量实测 + 主智能体 vitest cross-check（**apps/server it() = 1291 passed + 62 skipped = 1353 total** / funcs 90.64% / 36 module / 65 表 / 49 迁移 / **25 页 / 16 组件 沿用 init #18 一致** V0.2.39-79 0 新增 page / GAP-12 36/36 closed（V0.2.42 strength + apps/flutter 新建）；**V0.2.43-45 ai-coach 多模态识图前端改版**（pickMedia → upload COS → curImageUrl → streamChat 带 imageUrl → user 气泡 img-thumb，配合后端 ChatMessage.content 扩 ContentPart[] + GLM-4.6V）/ **V0.2.58 沐禾健康协议 + 改名**（15 处品牌名替换：config/env brandName + privacy-popup + 3 poster 海报 + ai-coach 欢迎/分享 + runner 数据中心/周报 + report-monthly/interpret/feed/membership 分享标题 + onboarding 完成语，**公司名「湖南青沐生命科技有限公司」保留**）/ **V0.2.60-61 interpret screenshot 用户确认**（识别卡+确认按钮防误识别污染跑量 + 前端体验版 upload 通过 1.1MB 佳明 FIT 样本走通）/ **V0.2.63 H5 fallback**（小程序上传失败引导 H5，issueH5Token Redis 5min + chooseMedia fallback modal + 小程序历史回看）/ **V0.2.66 提审 API**（admin getMpCategory/uploadMpMedia/submitMpAudit SUPER_ONLY + curl 一键发布）；**V0.2.67-72 Flutter APP**（独立仓 apps/flutter/CLAUDE.md V0.2.79 init #19 新建，~80 文件 / 21 feature / 4-tab / Riverpod+go_router+dio+M3 / com.qingmu.muhehealth / Phase 1-3 ✅ / Phase 1.5 微信 APP 接入预留）/ **V0.2.71 Terra 智能手表 hotfix**（device.routes 已 V0.1.130，新建重复路由致生产 FST_ERR_DUPLICATED_ROUTE crash → V0.2.71 删重复）；**0 小程序代码改动纯文档**；**WechatSI 插件状态**：V0.2.44 改系统输入法语音 / V0.2.25 临时移除 app.json plugins + scope.record，**GAP-18 open 待常智公众平台授权**；**miniprogram_npm 沿用 V0.2.42 rebuild**（V0.2.33+ ENDPOINTS.interpret / V0.2.42+ ENDPOINTS.strength 在 mini-program npm build 产物已含，pages/strength 落地需触发一次 rebuild if V0.2.42 后未 rebuild）；下一步：① miniprogram_npm rebuild（如上次 build 后 shared 有改动，本次 0 改动可免）；② pages/strength 落地 + 组间计时 UI；③ 提审发布 v0.2.66（mp 服务类目申请 + curl 一键提审）；④ WechatSI 授权加回恢复 K5 voice（GAP-18）

- **2026-07-21** — 🎯 **V0.2.43-45 ai-coach 前端改版（默认新聊 + 去 voice + 多模态识图）**：① **V0.2.43 默认新聊**（onLoad 去 loadHistory，进页显 quick cards + AI 气泡 + 快速提问，用户点「历史」才看历史会话）；② **V0.2.44 去 voice 按钮**（弃 WechatSI 插件依赖，改系统输入法语音输入，删 onTapVoice 68 行 + voice-btn wxml/wxss，**GAP-18 永久 open**）；③ **V0.2.45 多模态识图**（input 栏 +📷 pick-btn → chooseMedia → uploadFile COS → curImageUrl → streamChat 带 imageUrl → user 气泡 img-thumb 显示；Msg +imageUrl 字段 + input-bar 改 column 布局容纳缩略图；配合后端 ChatMessage.content 扩 ContentPart[] + GLM-4.6V 含图切 vision model）；**0 新页 / 0 新组件**（仅 ai-coach 页改）；**25 页 / 16 组件不变**；生产 v0.2.47 已部署（ai-coach 多模态上线）
