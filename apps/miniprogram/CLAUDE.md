...
├── app.json                        # 页面路由 + tabBar + 全局窗口配置（38 页面注册）
...
🤙 别在 tabBar 上反复纠结，先把 `services/api.ts` 跑通。**38 页面已成型**（V0.1.42 跑群深化 group-detail 改造 + V0.1.41 训练计划配置化 + V0.1.40 profile 完整；V0.1.37 团购 2 页 + V0.1.36 红心广场/话题 + V0.1.34 家庭空间），下一步：蓝牙真机联调（V0.1.33 品牌识别就位）+ ✅ 训练计划配置化（V0.1.41）+ ✅ 跑群深化（V0.1.42）+ 赛事服务 MVP（业务闭环第 3 块）+ 评价系统 + 年度报告跑鞋维度 + 目标/证书增强 + 收藏/动态社交向扩展 + 用户主页增强。
